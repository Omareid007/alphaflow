...
async function checkPortfolioRisk(): Promise<RiskMetrics> {
  ...
  for (const pos of positions) {
    ...
    sectorExposure[sector] = (sectorExposure[sector] || 0) + Math.abs(pos.marketValue);
  }
  ...
  const concentrationBreached = maxConcentration > RISK_THRESHOLDS.MAX_POSITION_CONCENTRATION;
  const sectorBreached = maxSectorExposure > RISK_THRESHOLDS.MAX_SECTOR_EXPOSURE;

  if (concentrationBreached) {
    alerts.push(`Position ${maxConcentrationSymbol} exceeds 5% limit (${(maxConcentration * 100).toFixed(1)}%)`);
    riskLevel = 'HIGH';
  } else if (maxConcentration > RISK_THRESHOLDS.MAX_POSITION_CONCENTRATION * RISK_THRESHOLDS.WARNING_THRESHOLD) {
    alerts.push(`Position ${maxConcentrationSymbol} approaching 5% limit (${(maxConcentration * 100).toFixed(1)}%)`);
    riskLevel = 'MEDIUM';
  }

  if (sectorBreached) {
    alerts.push(`Sector ${maxSectorName} exceeds 25% limit (${(maxSectorExposure * 100).toFixed(1)}%)`);
    riskLevel = riskLevel === 'HIGH' ? 'CRITICAL' : 'HIGH';
  } else if (maxSectorExposure > RISK_THRESHOLDS.MAX_SECTOR_EXPOSURE * RISK_THRESHOLDS.WARNING_THRESHOLD) {
    alerts.push(`Sector ${maxSectorName} approaching 25% limit (${(maxSectorExposure * 100).toFixed(1)}%)`);
    if (riskLevel === 'LOW') riskLevel = 'MEDIUM';
  }

  if (currentDrawdown > RISK_THRESHOLDS.MAX_DAILY_DRAWDOWN) {
    alerts.push(`Daily drawdown exceeds 5% limit (${(currentDrawdown * 100).toFixed(2)}%)`);
    riskLevel = 'CRITICAL';
  }
...
async function validateOrder(
  ...
  const newSectorExposure = (currentSectorExposure + (side === 'buy' ? estimatedValue : -estimatedValue)) / equity;

  const checks = {
    marketOpen: clock.is_open,
    symbolTradable: true, // Would need asset lookup for full validation
    sufficientBuyingPower: side === 'buy' ? estimatedValue <= buyingPower : true,
    positionLimitOk: newConcentration <= RISK_THRESHOLDS.MAX_POSITION_CONCENTRATION,
  ...
  if (!checks.sufficientBuyingPower) {
    failureReasons.push(`Insufficient buying power: need $${estimatedValue.toFixed(2)}, have $${buyingPower.toFixed(2)}`);
    recommendations.push(`Reduce order size to max ${Math.floor(buyingPower / estimatedPrice)} shares`);
  }

  if (!checks.positionLimitOk) {
    failureReasons.push(`Position would exceed 5% limit (${(newConcentration * 100).toFixed(1)}%)`);
    const maxQty = Math.floor((equity * RISK_THRESHOLDS.MAX_POSITION_CONCENTRATION - currentPositionValue) / estimatedPrice);
    recommendations.push(`Reduce quantity to max ${maxQty} shares`);
  }

  if (!checks.sectorLimitOk) {
    failureReasons.push(`Sector ${sector} would exceed 25% limit (${(newSectorExposure * 100).toFixed(1)}%)`);
  ...
  if (!checks.circuitBreakerOk) {
    ...
    recommendations.push(`Wait ${cbStatus.cooldownRemaining}s for circuit breaker reset`);
  }
...
async function checkCircuitBreaker(): Promise<CircuitBreakerStatus> {
  try {
    const response = await fetch(`${SERVER_URL}/api/alpaca/circuit-breaker/status`);
    if (!response.ok) {
      return {
        isOpen: false,
        openedAt: null,
...
const tools: Tool[] = [
  {
    ...
    description: 'Calculate comprehensive portfolio risk metrics including concentration, sector exposure, VaR, and drawdown. Returns alerts if any thresholds are breached.',
    inputSchema: {
      type: 'object',
      properties: {},
      required: [],
  ...
  {
    name: 'validate_order',
    description: 'Pre-trade validation against risk limits. Checks market hours, buying power, position limits (5%), sector exposure (25%), and circuit breaker state.',
    inputSchema: {
      type: 'object',
      properties: {
        symbol: {
  ...
  },
  {
    ...
    description: 'Get current portfolio positions with real-time P&L. Returns symbol, quantity, entry price, current price, market value, and unrealized P&L.',
    inputSchema: {
      type: 'object',
      properties: {},
      required: [],
  ...
  {
    name: 'market_status',
    description: 'Check market hours, next open/close times, and trading availability for equities and crypto.',
    inputSchema: {
      type: 'object',
      properties: {},
      required: [],
  ...
  },
  {
    ...
    description: 'Check trading circuit breaker status. Returns whether trading is suspended and time until reset.',
    inputSchema: {
      type: 'object',
      properties: {},
      required: [],
    ...
    try {
      switch (name) {
        ...
        case 'validate_order': {
          ...
          const result = await validateOrder(parsed.symbol, parsed.side, parsed.qty);
          return {
            content: [
              {
                type: 'text',
