...
describe("AI Pipeline Diagnostic", () => {
  ...
  it("should verify AI decisions contain actionable recommendations", () => {
    ...
    console.log(
      `   Non-actionable: ${recentDecisions.length - actionableDecisions.length}`
    );
  ...
  it("should verify trade execution follows AI decisions", () => {
    ...
    const decisionsWithoutOrders = [...decisionSymbols].filter(
      (s) => !orderedSymbols.has(s)
    );
  ...
  it("should verify agent status via API", async () => {
    try {
      ...
      console.log(
        `   Phase: ${agentStatus.phase || agentStatus.currentPhase || "unknown"}`
      );
      console.log(
        `   Last Update: ${agentStatus.lastUpdate || agentStatus.lastCycleTime || "unknown"}`
      );
  ...
  it("should trace full decision-to-execution pipeline", async () => {
    ...
    console.log(
      `Step 1: AI Decisions Generated: ${steps.aiDecisions ? "YES" : "NO"} (${recentDecisions.length})`
    );
    console.log(
      `Step 2: Orders Created: ${steps.orders ? "YES" : "NO"} (${recentOrders.length})`
    );
    console.log(
      `Step 3: Trades Executed: ${steps.trades ? "YES" : "NO"} (${recentTrades.length})`
    );

    const filledOrders = recentOrders.filter(
      (o) => o.status === "filled"
    ).length;
    ...
    if (healthPercent < 100) {
      console.log("\nFIXES NEEDED:");
      if (!steps.aiDecisions)
        console.log("   - Verify AI analysis is generating decisions");
      if (!steps.orders) console.log("   - Check order creation logic");
      if (!steps.trades)
        console.log("   - Verify trade execution and broker connectivity");
    }
  });
});
