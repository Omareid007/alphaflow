 ...
 */
export const debateConsensus = pgTable(
  "debate_consensus",
  {
    id: varchar("id")
      .primaryKey()
...
export const aiOutcomeLinks = pgTable(
  "ai_outcome_links",
  {
    ...
    debateSessionId: varchar("debate_session_id").references(
      () => debateSessions.id,
      { onDelete: "set null" }
    ),
    ...
    outcome: text("outcome")
      .$type<"win" | "loss" | "breakeven" | "open" | "unknown">()
      .default("unknown"),
...
export const insertDebateSessionSchema = createInsertSchema(
  debateSessions
).omit({
  id: true,
  createdAt: true,
});
export const insertDebateMessageSchema = createInsertSchema(
  debateMessages
).omit({
  id: true,
  createdAt: true,
});
export const insertDebateConsensusSchema = createInsertSchema(
  debateConsensus
).omit({
  id: true,
  createdAt: true,
...
export const insertAiAgentProfileSchema = createInsertSchema(
  aiAgentProfiles
).omit({
  id: true,
  createdAt: true,
...
export const insertAiOutcomeLinkSchema = createInsertSchema(
  aiOutcomeLinks
).omit({
  id: true,
  createdAt: true,
