...
class FrankfurterConnector {
   ...
   */
  async getLatestRates(base: string = "USD", symbols?: string[]): Promise<ExchangeRates | null> {
   ...
   */
  async getRateOnDate(base: string, quote: string, date: string): Promise<number | null> {
    try {
  ...
  async getCurrencies(): Promise<CurrencyInfo[]> {
    ...
    try {
      ...
      const currencies: CurrencyInfo[] = Object.entries(data).map(([code, name]) => ({
        code,
        name,
      }));
   ...
   */
  async getForexPairSummary(base: string, quote: string): Promise<ForexSummary | null> {
    try {
      // Get current rate
  ...
  async getUSDStrengthIndex(): Promise<{
    index: number;
    components: { currency: string; weight: number; rate: number; contribution: number }[];
