...
export function hasCapability(context: RBACContext, capability: AdminCapability): boolean {
  return context.capabilities.includes(capability);
}

export function hasAnyCapability(context: RBACContext, capabilities: AdminCapability[]): boolean {
  return capabilities.some(cap => context.capabilities.includes(cap));
}

export function hasAllCapabilities(context: RBACContext, capabilities: AdminCapability[]): boolean {
  return capabilities.every(cap => context.capabilities.includes(cap));
}

export function createRBACContext(user: { id: string; username: string; isAdmin: boolean }): RBACContext {
...
export function filterModulesByCapability<T extends { capability: AdminCapability }>(
  modules: T[],
  context: RBACContext
): T[] {
...
export function getRoleInfo(role: UserRole): { role: UserRole; capabilities: AdminCapability[]; description: string } {
