...
const inMemoryCache: Map<string, { data: unknown; expiresAt: number; staleUntilAt: number }> = new Map();

function getCacheKey(provider: string, key: string): string {
  return `${provider}:${key}`;
}

export async function getFromCache<T>(provider: string, key: string): Promise<CacheResult<T> | null> {
  ...
  if (memCached) {
    if (now < memCached.expiresAt) {
      return { data: memCached.data as T, isFresh: true, isStale: false, hitCount: 0 };
    }
    if (now < memCached.staleUntilAt) {
      return { data: memCached.data as T, isFresh: false, isStale: true, hitCount: 0 };
    }
...
export async function setInCache<T>(provider: string, key: string, data: T): Promise<void> {
...
export async function invalidateCache(provider: string, key?: string): Promise<number> {
  let deletedCount = 0;
...
export async function getCacheStats(provider?: string): Promise<CacheStats> {
  ...
  try {
    ...
    if (provider) {
      query = query.where(eq(externalApiCacheEntries.provider, provider)) as typeof query;
    }
...
export async function getAllCacheEntries(provider?: string): Promise<Array<{
  provider: string;
  cacheKey: string;
  expiresAt: Date;
  staleUntilAt: Date;
