...
export const MOVING_AVERAGE_PRESETS: MovingAveragePreset[] = [
  {
    ...
    riskLimitPct: 0.05,
    description:
      "Lower risk with wider SMA gaps, smaller positions. Best for beginners.",
  },
  {
    id: "balanced",
    name: "Balanced",
    ...
    riskLimitPct: 0.1,
    description:
      "Classic 7/20 SMA crossover. Good balance of signals and reliability.",
  },
  {
    id: "aggressive",
    name: "Aggressive",
    ...
    riskLimitPct: 0.15,
    description:
      "Faster signals with tighter SMAs. More trades, higher volatility.",
  },
];
...
export const STRATEGY_SCHEMA = {
  ...
  description:
    "A simple trend-following strategy that generates buy signals when a short-term moving average crosses above a longer-term moving average, and sell signals when it crosses below. The classic 7/20 SMA version has historically shown ~21% annualized returns in backtests.",
...
export function normalizeMovingAverageConfig(
  ...
  const preset = MOVING_AVERAGE_PRESETS.find(
  ...
  const config: MovingAverageCrossoverConfig = {
    ...
    adaptiveRiskIntervalMinutes:
      input.adaptiveRiskIntervalMinutes ?? ADAPTIVE_DEFAULTS.intervalMinutes,
...
export async function backtestMovingAverageStrategy(
  ...
  const { updateStrategyRiskIfNeeded } =
    await import("./adaptive-risk-service");
  ...
  const smaCrossoverPoints: Array<{
    date: string;
    type: "bullish" | "bearish";
    price: number;
  }> = [];
  ...
  for (const crossover of crossovers) {
    const date = new Date(timestamps[crossover.index] * 1000)
      .toISOString()
      .split("T")[0];
  ...
  for (let i = 0; i < closePrices.length; i++) {
    ...
    if (crossover?.type === "bullish" && !inPosition) {
      ...
      equity =
        entryEquity * (1 + returnPct * normalizedConfig.allocationPct * 10);
      inPosition = false;
    } else if (inPosition) {
      const unrealizedPct = (price - entryPrice) / entryPrice;
      const currentEquity =
        entryEquity * (1 + unrealizedPct * normalizedConfig.allocationPct * 10);
