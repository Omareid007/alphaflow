...
export const PARAMETER_BOUNDS = {
  ...
  riskLimitPct: { min: 0.01, max: 0.10, default: 0.03 },
...
export function normalizeMeanReversionConfig(
  ...
  deviationThreshold = Math.max(PARAMETER_BOUNDS.deviationThreshold.min, Math.min(PARAMETER_BOUNDS.deviationThreshold.max, deviationThreshold));
  ...
  maxHoldingPeriod = Math.max(PARAMETER_BOUNDS.maxHoldingPeriod.min, Math.min(PARAMETER_BOUNDS.maxHoldingPeriod.max, Math.round(maxHoldingPeriod)));
...
export interface MeanReversionBacktestResult {
  ...
  bandCrossings: Array<{ date: string; type: "oversold" | "overbought"; price: number; zScore: number }>;
}
...
export async function backtestMeanReversionStrategy(
  ...
  const bandCrossings: Array<{ date: string; type: "oversold" | "overbought"; price: number; zScore: number }> = [];
  
  let position: { entryIndex: number; entryPrice: number; side: "long" | "short" } | null = null;

  for (const crossing of crossings) {
    const date = new Date(timestamps[crossing.index] * 1000).toISOString().split("T")[0];
  ...
  if (position) {
    for (let i = position.entryIndex + 1; i < closePrices.length; i++) {
      ...
      if (holdingDays >= normalizedConfig.maxHoldingPeriod) {
        ...
        const entryDate = new Date(timestamps[position.entryIndex] * 1000).toISOString().split("T")[0];
        const exitDate = new Date(timestamps[i] * 1000).toISOString().split("T")[0];
  ...
  for (let i = 0; i < closePrices.length; i++) {
    ...
    if (!inPosition && z !== null && z <= -normalizedConfig.deviationThreshold) {
      inPosition = true;
      entryPrice = price;
      entryEquity = equity;
      positionSide = "long";
