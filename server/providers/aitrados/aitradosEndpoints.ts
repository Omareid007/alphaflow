...
function getBaseUrl(): string {
  return (
    process.env.AITRADOS_BASE_URL || "https://default.dataset-api.aitrados.com"
  );
}
...
export function buildOhlcLatestUrl(params: OhlcLatestParams): string {
  ...
  const url = new URL(
    `${base}/api/v2/${params.schemaAsset}/bars/${params.countrySymbol}/${params.interval}/latest`
  );
...
export function buildEconomicEventUrl(
