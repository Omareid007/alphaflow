...
import { db } from "../db";
import { workItems, llmCalls, aiDecisions, trades, agentStatus, alertRules, alertEvents } from "@shared/schema";
  ...
  try {
    ...
    const [
      ...
      db.select({
        total: count(),
        errors: sql<number>`count(case when ${llmCalls.status} = 'error' then 1 end)`,
      }).from(llmCalls).where(gte(llmCalls.createdAt, oneDayAgo)),
    ...
    const oldestPendingAge = oldestPending[0]?.createdAt
      ? Math.round((now.getTime() - new Date(oldestPending[0].createdAt).getTime()) / 60000)
      : 0;
...
observabilityRouter.get("/metrics/summary", async (req: Request, res: Response) => {
  try {
...
observabilityRouter.get("/work-queue/items", async (req: Request, res: Response) => {
  try {
...
observabilityRouter.get("/work-queue/items/:id", async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
observabilityRouter.post("/work-queue/items/:id/retry", async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
observabilityRouter.post("/work-queue/items/:id/cancel", async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const item = await workQueueService.getById(id);
...
observabilityRouter.post("/work-queue/items/:id/dead-letter", async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
observabilityRouter.get("/trace/:traceId", async (req: Request, res: Response) => {
...
observabilityRouter.get("/alerts/rules", async (req: Request, res: Response) => {
  try {
...
observabilityRouter.post("/alerts/rules", async (req: Request, res: Response) => {
  try {
...
observabilityRouter.post("/alerts/rules/:id/toggle", async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
observabilityRouter.delete("/alerts/rules/:id", async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
observabilityRouter.post("/alerts/test", async (req: Request, res: Response) => {
  try {
...
observabilityRouter.get("/alerts/events", async (req: Request, res: Response) => {
  try {
...
observabilityRouter.post("/alerts/evaluate", async (req: Request, res: Response) => {
  try {
