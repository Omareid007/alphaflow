    ...
    try {
      ...
      if (provider.toLowerCase() === "valyu" && !confirmValyu) {
        return res.status(400).json({
          ...
          message:
            "Set confirmValyu: true to force refresh Valyu data (1 call/week limit)",
        });
      }
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (webRetrievalsPerMonth !== undefined)
        updates.webRetrievalsPerMonth = webRetrievalsPerMonth;
      if (financeRetrievalsPerMonth !== undefined)
        updates.financeRetrievalsPerMonth = financeRetrievalsPerMonth;
      if (proprietaryRetrievalsPerMonth !== undefined)
    ...
    try {
      ...
      const connectors = [
        ...
        },
        {
          ...
          status: providerStatuses.coinmarketcap?.enabled
            ? "active"
            : "disabled",
          lastSync: null,
          callsRemaining: null,
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const getPolicyEnabled = (provider: string): boolean => {
        const policy = providerPolicies.find(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const dataSources = [
        {
          ...
          provider: "finnhub",
          active:
            !!process.env.FINNHUB_API_KEY && providerStatuses.finnhub?.enabled,
          category: "market_data",
        },
        {
          name: "Crypto Prices",
        ...
        },
        {
          ...
          active:
            !!(process.env.ALPACA_API_KEY && process.env.ALPACA_SECRET_KEY) &&
            providerStatuses.alpaca?.enabled,
          category: "brokerage",
        },
        {
        ...
        },
        {
          ...
          provider: "valyu",
          active:
            !!process.env.VALYU_API_KEY && providerStatuses.valyu?.enabled,
          category: "fundamentals",
        },
        {
        ...
        },
        {
          ...
          provider: "newsapi",
          active:
            !!process.env.NEWS_API_KEY && providerStatuses.newsapi?.enabled,
          category: "news",
        },
        {
          ...
          active:
            ...
            providerStatuses.huggingface?.enabled,
          category: "sentiment",
        },
      ];

      const activeSourcesCount =
        marketIntelligence?.activeSources ??
        dataSources.filter((s) => s.active).length;
      const totalSources =
        marketIntelligence?.totalSources ?? dataSources.length;
      const intelligenceScore =
        marketIntelligence?.overall ?? activeSourcesCount / totalSources;
      ...
      const fusionMetrics = {
        ...
        capabilities: {
          marketData:
            dataSources.some((s) => s.provider === "finnhub" && s.active) ||
            dataSources.some((s) => s.provider === "coingecko" && s.active),
          newsAnalysis: dataSources.some(
            (s) =>
              (s.provider === "gdelt" || s.provider === "newsapi") && s.active
          ),
          ...
          fundamentals: dataSources.some(
            (s) =>
              (s.provider === "sec-edgar" || s.provider === "valyu") && s.active
          ),
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const maskedEnvKey = envApiKey
        ? `${envApiKey.substring(0, 6)}...${envApiKey.substring(envApiKey.length - 4)}`
        : "NOT SET";

      // Try to read .env file values for comparison
      let envFileKey = null;
      try {
      ...
      const maskedEnvFileKey = envFileKey
        ? `${envFileKey.substring(0, 6)}...${envFileKey.substring(envFileKey.length - 4)}`
        : "NOT FOUND";
      ...
      res.json({
        ...
        credentials: {
          ...
          mismatchWarning: hasMismatch
            ? "WARNING: Running environment uses different credentials than .env file. This may indicate Replit Secrets are overriding .env values."
            : null,
        },
        account: accountInfo,
        connectionStatus,
