...
export const analysisFeedback = pgTable("analysis_feedback", {
  id: varchar("id")
    .primaryKey()
    .default(sql`gen_random_uuid()`),
  dataSourceAnalysisId: varchar("data_source_analysis_id").references(() => dataSourceAnalysis.id),
  tradeOutcomeId: varchar("trade_outcome_id").references(() => aiTradeOutcomes.id),
  symbol: text("symbol").notNull(),
