...
const logger = {
  ...
  error: (msg: string, data?: any) =>
    log.error("DynamicRiskManager", msg, data),
};
...
export class DynamicRiskManager {
  ...
  async getAdjustedLimits(
    ...
    if (this.config.performanceScaling && recentTrades) {
      const perfScaling = this.getPerformanceScaling(
        portfolioSnapshot,
        recentTrades
      );
    ...
    const combinedFactor = Object.values(scalingFactors).reduce(
      (a, b) => a * b,
      1.0
    );
    ...
    const adjustedOrdersPerDay = Math.max(
      1,
      Math.floor(baseOrdersPerDay * combinedFactor)
    );
   ...
   */
  private async getVolatilityScaling(): Promise<{
    factor: number;
    reason: string;
  }> {
    ...
    switch (regime) {
      ...
      case TimeRegime.MARKET_CLOSE:
        ...
        return {
          factor: 0.7,
          reason:
            "Near market close (15:00-16:00 ET), -30% to reduce overnight risk",
        };
   ...
   */
  async shouldAllowNewPositions(): Promise<{
    allowed: boolean;
    reason: string;
  }> {
    // Check VIX level
  ...
  async getDiagnostics(): Promise<{
    ...
    const volatilityRegime =
      vix !== null ? this.classifyVolatilityRegime(vix) : "unknown";
