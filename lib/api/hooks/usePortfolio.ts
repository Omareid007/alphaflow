...
export function usePositions() {
  return useQuery({
    queryKey: ['positions'],
    queryFn: async () => {
      const response = await api.get<{ positions: any[] } | any[]>('/api/positions');
      ...
      return rawPositions.map((p: any): Position => ({
        id: p.id || p.asset_id || '',
        symbol: p.symbol || '',
        ...
        unrealizedPl: p.unrealizedPl ?? p.unrealizedPnl ?? p.unrealized_pl ?? 0,
        // CRITICAL FIX: Handle unrealizedPnlPercent (from EnrichedPosition, already in %) and unrealized_plpc (from Alpaca, needs *100)
        unrealizedPlPct: p.unrealizedPlPct ?? p.unrealizedPnlPercent ?? (p.unrealized_plpc ? p.unrealized_plpc * 100 : 0),
...
export function useTrades(options?: { limit?: number; offset?: number }) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      return response.map((t: any): Trade => ({
        id: t.id || '',
        symbol: t.symbol || '',
        side: t.side || 'buy',
        qty: parseFloat(t.quantity) || parseFloat(t.qty) || 0,
        price: parseFloat(t.price) || 0,
        totalValue: (parseFloat(t.quantity) || parseFloat(t.qty) || 0) * (parseFloat(t.price) || 0),
