...
class ResourceMonitor {
  ...
  async report() {
    ...
    const avgHeapUsed =
      memMetrics.reduce((sum, m) => sum + m.heapUsed, 0) / memMetrics.length;
    const maxHeapUsed = Math.max(...memMetrics.map((m) => m.heapUsed));
    const minHeapUsed = Math.min(...memMetrics.map((m) => m.heapUsed));

    const avgRss =
      memMetrics.reduce((sum, m) => sum + m.rss, 0) / memMetrics.length;
    const maxRss = Math.max(...memMetrics.map((m) => m.rss));

    console.log(
      `  Heap Used (Avg):     ${(avgHeapUsed / 1024 / 1024).toFixed(2)} MB`
    );
    console.log(
      `  Heap Used (Max):     ${(maxHeapUsed / 1024 / 1024).toFixed(2)} MB`
    );
    console.log(
      `  Heap Used (Min):     ${(minHeapUsed / 1024 / 1024).toFixed(2)} MB`
    );
    console.log(
      `  RSS (Avg):           ${(avgRss / 1024 / 1024).toFixed(2)} MB`
    );
    console.log(
      `  RSS (Max):           ${(maxRss / 1024 / 1024).toFixed(2)} MB`
    );
    ...
    const avgTotal =
      dbMetrics.reduce((sum, m) => sum + m.totalConnections, 0) /
      dbMetrics.length;
    const maxTotal = Math.max(...dbMetrics.map((m) => m.totalConnections));
    const avgActive =
      dbMetrics.reduce((sum, m) => sum + m.activeConnections, 0) /
      dbMetrics.length;
    ...
    if (maxTotal > 50) {
      console.log(
        "  ⚠️  WARNING: High connection count - risk of connection pool exhaustion"
      );
    } else if (maxTotal > 20) {
    ...
    try {
      const { stdout: cpuInfo } = await execAsync(
        'top -b -n 1 | grep "node\\|Cpu" | head -5'
      );
    ...
    try {
      const { stdout: memInfo } = await execAsync(
        'free -m 2>/dev/null || echo "Not available"'
      );
