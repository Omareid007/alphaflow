...
async function analyzeQuery(query: string, label: string): Promise<QueryPlan> {
  ...
  try {
    ...
    const hasIndexScan = planText.toLowerCase().includes('index scan') ||
    ...
    console.log(`   🔍 Scan Type: ${hasSeqScan ? '⚠️ Sequential Scan' : hasIndexScan ? '✅ Index Scan' : '❓ Unknown'}`);

    if (hasSeqScan && rowsReturned > 100) {
      console.log(`   🚨 WARNING: Sequential scan on ${rowsReturned} rows - consider adding an index!`);
    }
...
async function testDatabasePerformance() {
  ...
  const tableQueries = [
    ...
    { table: 'ai_decisions', column: 'strategy_id', reason: 'Frequently joined' },
  ...
  const missingIndexes: Array<{ table: string; column: string; reason: string }> = [];

  for (const { table, column, reason } of tableQueries) {
    const indexes = existingIndexes.get(table) || [];
    const hasIndex = indexes.some(idx =>
      idx.includes(`(${column})`) || idx.includes(`(${column},`) || idx.includes(`, ${column}`)
    );
  ...
  // Row counts
  const tables = ['users', 'sessions', 'strategies', 'trades', 'positions',
                  'ai_decisions', 'orders', 'fills', 'backtest_runs'];
  ...
  for (const table of tables) {
    try {
      ...
      console.log(`  ${status} ${table.padEnd(35)} ${count.toLocaleString()} rows`);
    } catch (error) {
  ...
  const recommendations: Array<{ priority: string; title: string; description: string; impact: string }> = [];
  ...
  if (missingIndexes.length > 0) {
    ...
    missingIndexes.forEach(({ table, column }) => {
      console.log(`CREATE INDEX CONCURRENTLY ${table}_${column}_idx ON ${table}(${column});`);
    });
  }
