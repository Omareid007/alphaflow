...
`;
  ...
  logProgress(generation: number, totalGens: number, metrics: Record<string, number>): void {
    ...
    this.log(`Gen ${generation.toString().padStart(4)}/${totalGens} ${bar} ${pct}% | Best: ${metrics.bestFitness?.toFixed(2) || 'N/A'} | Avg: ${metrics.avgFitness?.toFixed(2) || 'N/A'} | Rate: ${metrics.evalRate?.toFixed(1) || '0'}/s | ${elapsed}`);
  }
  ...
  logBest(genome: ProductionGenome, result: BacktestResult): void {
    ...
    this.log('═'.repeat(70));
    this.log(`   Fitness: ${genome.fitness.toFixed(4)} | Sharpe: ${result.sharpe.toFixed(4)} | Return: ${(result.totalReturn * 100).toFixed(2)}%`);
    this.log(`   MaxDD: ${(result.maxDrawdown * 100).toFixed(2)}% | WinRate: ${(result.winRate * 100).toFixed(1)}% | Trades: ${result.trades}`);
    this.log('═'.repeat(70) + '\n');
  }

  logInsight(insight: LearningInsight): void {
    this.log(`   💡 ${insight.pattern} (correlation: ${insight.correlation.toFixed(3)})`);
  }
...
class ContinuousLearningEngine {
  ...
  analyzePopulation(population: ProductionGenome[]): LearningInsight[] {
    ...
    for (const param of Object.keys(PARAM_RANGES)) {
      ...
      if (Math.abs(diff) > 0.15) {
        newInsights.push({
          ...
          correlation: diff, sampleSize: top10Pct.length,
          avgImprovement: top10Pct[0].fitness - (bottom10Pct[bottom10Pct.length - 1]?.fitness || 0),
          confidence: Math.min(1, Math.abs(diff) * top10Pct.length / 20), timestamp: new Date(),
        });
      }
    }
    ...
    for (const genome of top10Pct) {
      ...
      if (!currentBest || genome.fitness > currentBest.fitness) this.bestByRegime.set(regime, genome);
    }
  ...
  getAdaptiveMutationRate(generation: number, avgFitness: number): number {
    ...
    if (Math.abs(improvement) < CONFIG.CONVERGENCE_THRESHOLD) return Math.min(CONFIG.MUTATION_RATE_INITIAL * 2.5, 0.4);
    if (improvement > 0.05) return Math.max(CONFIG.MUTATION_RATE_INITIAL * 0.6, 0.06);
  ...
  suggestGuidedMutation(genome: ProductionGenome): Record<string, number> {
    ...
    for (const [param, correlation] of this.parameterCorrelations) {
      if (Math.abs(correlation) > 0.2 && PARAM_RANGES[param]) {
        ...
        suggestions[param] = Math.max(range.min, Math.min(range.max, (genome.genes[param] || range.min) + direction * range.step * 2));
      }
    }
    return suggestions;
  }
  ...
  getBestByRegime(): Map<string, ProductionGenome> { return this.bestByRegime; }
...
class JudgeSystem {
  evaluate(genome: ProductionGenome, result: BacktestResult): { verdict: string; warnings: string[]; score: number } {
    const warnings: string[] = [];
    let score = Math.min(result.sharpe, 3) * 20 + Math.min(result.sortino, 4) * 10 + Math.min(result.calmar, 3) * 15 +
      result.winRate * 20 + Math.min(result.totalReturn * 2, 40) + (1 - result.maxDrawdown) * 30 +
      Math.min(result.trades / 500, 1) * 10 + Math.min(result.profitFactor, 3) * 15;

    if (result.maxDrawdown > 0.25) { score -= (result.maxDrawdown - 0.25) * 100; warnings.push(`High drawdown: ${(result.maxDrawdown * 100).toFixed(1)}%`); }
    if (result.trades < 30) { score -= (30 - result.trades) * 2; warnings.push(`Low trades: ${result.trades}`); }
    if (result.sharpe > 4) { score -= 30; warnings.push('OVERFITTING: Sharpe > 4'); }
    if (result.winRate > 0.85 && result.trades > 50) { score -= 20; warnings.push('OVERFITTING: Win rate > 85%'); }

    const verdict = warnings.some(w => w.includes('OVERFITTING')) ? 'SUSPICIOUS' : score > 180 ? 'EXCELLENT' : score > 140 ? 'GOOD' : score > 90 ? 'ACCEPTABLE' : 'POOR';
...
function detectMarketRegime(closes: number[], lookback: number = 50): string {
  ...
  if (momentum > 0.1 && current > sma20[sma20.length - 1] && sma20[sma20.length - 1] > sma50[sma50.length - 1]) return volatility > 0.25 ? 'volatile_bull' : 'strong_bull';
  if (momentum < -0.1 && current < sma20[sma20.length - 1] && sma20[sma20.length - 1] < sma50[sma50.length - 1]) return volatility > 0.25 ? 'volatile_bear' : 'strong_bear';
...
function generateSignal(bars: AlpacaBar[], genes: Record<string, number>, idx: number): { score: number; confidence: number } {
  ...
  factors.technical = !isNaN(currentRSI) ? (currentRSI < (genes.rsiOversold || 30) ? 0.8 : currentRSI > (genes.rsiOverbought || 70) ? -0.8 : (50 - currentRSI) / 50) : 0;

  const macd = calculateMACD(closes, genes.macdFast || 12, genes.macdSlow || 26, genes.macdSignal || 9);
  if (macd.histogram.length > 1) factors.technical += macd.histogram[macd.histogram.length - 1] > macd.histogram[macd.histogram.length - 2] ? 0.3 : -0.3;
  factors.technical = Math.max(-1, Math.min(1, factors.technical));

  const momShort = genes.momentumShort || 5, momMed = genes.momentumMedium || 20;
  factors.momentum = closes.length > momMed ? Math.max(-1, Math.min(1, ((closes[closes.length - 1] - closes[closes.length - momShort - 1]) / closes[closes.length - momShort - 1]) * 10 + ((closes[closes.length - 1] - closes[closes.length - momMed - 1]) / closes[closes.length - momMed - 1]) * 5)) : 0;

  const atr = calculateATR(highs, lows, closes, genes.atrPeriod || 14);
  factors.volatility = !isNaN(atr[atr.length - 1]) ? Math.max(-1, Math.min(1, 0.5 - (atr[atr.length - 1] / closes[closes.length - 1]) * 20)) : 0;

  factors.volume = volumes.length > 20 ? Math.max(-1, Math.min(1, (volumes[volumes.length - 1] / (volumes.slice(-21, -1).reduce((a, b) => a + b, 0) / 20) - 1) * 0.5)) : 0;
  factors.sentiment = Math.max(-1, Math.min(1, (closes.length > 5 ? (closes[closes.length - 1] - closes[closes.length - 6]) / closes[closes.length - 6] : 0) * 10));

  const bb = calculateBollingerBands(closes, genes.bbPeriod || 20, genes.bbStdDev || 2);
  const current = closes[closes.length - 1];
  factors.pattern = bb.upper[bb.upper.length - 1] && bb.lower[bb.lower.length - 1] ? (current < bb.lower[bb.lower.length - 1] ? 0.6 : current > bb.upper[bb.upper.length - 1] ? -0.6 : 0) : 0;

  const smaShort = calculateSMA(closes, genes.smaShort || 10);
  const smaMed = calculateSMA(closes, genes.smaMedium || 50);
  factors.breadth = !isNaN(smaShort[smaShort.length - 1]) && !isNaN(smaMed[smaMed.length - 1]) ? Math.max(-1, Math.min(1, (current > smaShort[smaShort.length - 1] ? 0.5 : -0.5) + (current > smaMed[smaMed.length - 1] ? 0.5 : -0.5))) : 0;
  factors.correlation = bb.middle && !isNaN(bb.middle[bb.middle.length - 1]) ? Math.max(-1, Math.min(1, -((current - bb.middle[bb.middle.length - 1]) / ((bb.upper[bb.upper.length - 1] - bb.middle[bb.middle.length - 1]) || 1)) * 0.5)) : 0;

  const score = factors.technical * (genes.technicalWeight || 0.2) + factors.momentum * (genes.momentumWeight || 0.2) + factors.volatility * (genes.volatilityWeight || 0.1) + factors.volume * (genes.volumeWeight || 0.1) + factors.sentiment * (genes.sentimentWeight || 0.1) + factors.pattern * (genes.patternWeight || 0.1) + factors.breadth * (genes.breadthWeight || 0.1) + factors.correlation * (genes.correlationWeight || 0.1);
  const factorValues = Object.values(factors);
  const confidence = (Math.max(factorValues.filter(f => f > 0.2).length, factorValues.filter(f => f < -0.2).length) / factorValues.length) * Math.abs(score);
...
function runBacktest(genome: ProductionGenome, barsMap: Map<string, AlpacaBar[]>, symbols: string[], startIdx: number, endIdx: number): BacktestResult {
  ...
  for (let idx = startIdx; idx < Math.min(endIdx, refBars.length); idx++) {
    const regime = detectMarketRegime(refBars.slice(0, idx + 1).map(b => b.c), genes.regimeLookback || 50);
    ...
    for (const [symbol, pos] of positions) {
      ...
      if (exitPrice) {
        trades.push({ pnl: (exitPrice - pos.entry) * pos.shares, holdingDays: idx - pos.entryIdx });
    ...
    if (positions.size < (genes.maxPositions || 20)) {
      ...
      for (const symbol of symbols) {
        ...
        if (signal.score >= (genes.buyThreshold || 0.12) && signal.confidence >= (genes.confidenceMin || 0.28)) {
          ...
          const atr = calculateATR(symbolBars.slice(0, idx + 1).map(b => b.h), symbolBars.slice(0, idx + 1).map(b => b.l), closes, genes.atrPeriod || 14);
          if (!isNaN(atr[atr.length - 1])) candidates.push({ symbol, score: signal.score, price: symbolBars[idx].c, atr: atr[atr.length - 1] });
        }
      }
      candidates.sort((a, b) => b.score - a.score);
      for (const c of candidates.slice(0, (genes.maxPositions || 20) - positions.size)) {
        const shares = Math.floor((capital * (genes.maxPositionPct || 0.05)) / c.price);
        if (shares > 0 && shares * c.price <= capital) {
          positions.set(c.symbol, { entry: c.price, shares, entryIdx: idx, stopLoss: c.price - c.atr * (genes.atrMultStop || 1.5), takeProfit: c.price + c.atr * (genes.atrMultTarget || 4) });
    ...
    for (const [symbol, pos] of positions) {
      const symbolBars = barsMap.get(symbol);
      if (symbolBars && idx < symbolBars.length) currentEquity += pos.shares * symbolBars[idx].c;
    }
  ...
  for (const [symbol, pos] of positions) {
    const symbolBars = barsMap.get(symbol);
    if (symbolBars?.length) trades.push({ pnl: (symbolBars[Math.min(endIdx - 1, symbolBars.length - 1)].c - pos.entry) * pos.shares, holdingDays: endIdx - pos.entryIdx });
  }

  return calculateMetrics(equity, dailyReturns, trades, regimeReturns, initialCapital);
}

function calculateMetrics(equity: number[], dailyReturns: number[], trades: { pnl: number; holdingDays: number }[], regimeReturns: Record<string, number[]>, initialCapital: number): BacktestResult {
  const finalEquity = equity[equity.length - 1];
  const totalReturn = (finalEquity - initialCapital) / initialCapital;
  let maxDrawdown = 0, peak = equity[0];
  for (const val of equity) { if (val > peak) peak = val; maxDrawdown = Math.max(maxDrawdown, (peak - val) / peak); }

  const avgReturn = dailyReturns.length > 0 ? dailyReturns.reduce((a, b) => a + b, 0) / dailyReturns.length : 0;
  const stdReturn = dailyReturns.length > 1 ? Math.sqrt(dailyReturns.reduce((sum, r) => sum + Math.pow(r - avgReturn, 2), 0) / dailyReturns.length) : 1;
  const sharpe = stdReturn > 0 ? (avgReturn * 252) / (stdReturn * Math.sqrt(252)) : 0;
  const negReturns = dailyReturns.filter(r => r < 0);
  const sortino = negReturns.length > 0 ? (avgReturn * 252) / (Math.sqrt(negReturns.reduce((sum, r) => sum + r * r, 0) / negReturns.length) * Math.sqrt(252)) : 0;
  const years = dailyReturns.length / 252;
  const calmar = maxDrawdown > 0 ? (years > 0 ? Math.pow(1 + totalReturn, 1 / years) - 1 : 0) / maxDrawdown : 0;
  ...
  for (const [regime, returns] of Object.entries(regimeReturns)) {
    if (returns.length > 0) {
      ...
      regimePerformance[regime] = { return: returns.reduce((a, b) => a + b, 0), sharpe: regimeStd > 0 ? (regimeAvg * 252) / (regimeStd * Math.sqrt(252)) : 0 };
    }
  }
...
function calculateFitness(result: BacktestResult): number {
  ...
  return result.sharpe * 25 + result.sortino * 15 + result.calmar * 20 + result.winRate * 15 + result.totalReturn * 15 + (1 - result.maxDrawdown) * 10 + Math.min(result.profitFactor, 3) * 10 + Math.min(result.trades / 300, 1) * 5;
}
  ...
  for (let i = 0; i < symbols.length; i += 5) {
    ...
    await Promise.all(batch.map(async (symbol) => {
      try {
        ...
        if (symbolBars.length > 100) { bars.set(symbol, symbolBars); loaded++; }
        else failed++;
      } catch { failed++; }
    }));
...
function saveCheckpoint(state: OptimizationState): void {
  fs.writeFileSync(CHECKPOINT_FILE, JSON.stringify({ generation: state.generation, totalEvaluations: state.totalEvaluations, globalBest: state.globalBest, globalBestResult: state.globalBestResult, timestamp: new Date().toISOString() }, null, 2));
...
function saveResults(state: OptimizationState, learningEngine: ContinuousLearningEngine): void {
  fs.writeFileSync(RESULTS_FILE, JSON.stringify({ summary: { totalEvaluations: state.totalEvaluations, generations: state.generation, runtime: Date.now() - state.startTime.getTime() }, globalBest: state.globalBest, globalBestResult: state.globalBestResult, insights: learningEngine.getInsights(), bestByRegime: Object.fromEntries(learningEngine.getBestByRegime()), timestamp: new Date().toISOString() }, null, 2));
...
function generateRandomGenome(generation: number, island: number): ProductionGenome {
  const genes: Record<string, number> = {};
  for (const [param, range] of Object.entries(PARAM_RANGES)) {
    let value = range.min + Math.floor(Math.random() * ((range.max - range.min) / range.step + 1)) * range.step;
    if (range.integer) value = Math.round(value);
    genes[param] = value;
  }
  normalizeWeights(genes);
  return { id: `g${generation}-i${island}-${Math.random().toString(36).substr(2, 8)}`, genes, fitness: 0, sharpe: 0, sortino: 0, calmar: 0, winRate: 0, totalReturn: 0, maxDrawdown: 0, trades: 0, generation, island, parentIds: [], mutations: [], regime: 'unknown', evaluationTime: 0 };
}

function crossover(parent1: ProductionGenome, parent2: ProductionGenome, generation: number, island: number): ProductionGenome {
  const childGenes: Record<string, number> = {};
  for (const param of Object.keys(PARAM_RANGES)) {
    const rand = Math.random();
    if (rand < 0.35) childGenes[param] = parent1.genes[param];
    else if (rand < 0.70) childGenes[param] = parent2.genes[param];
    else {
  ...
  return { id: `g${generation}-i${island}-${Math.random().toString(36).substr(2, 8)}`, genes: childGenes, fitness: 0, sharpe: 0, sortino: 0, calmar: 0, winRate: 0, totalReturn: 0, maxDrawdown: 0, trades: 0, generation, island, parentIds: [parent1.id, parent2.id], mutations: [], regime: 'unknown', evaluationTime: 0 };
}

function mutate(genome: ProductionGenome, mutationRate: number, learningEngine?: ContinuousLearningEngine): ProductionGenome {
  ...
  for (const [param, range] of Object.entries(PARAM_RANGES)) {
    if (Math.random() < mutationRate) {
      let newVal: number;
      if (suggestions[param] !== undefined && Math.random() < 0.3) { newVal = suggestions[param]; mutations.push(`${param}: guided`); }
      else {
        newVal = mutatedGenes[param] + (Math.random() - 0.5) * 2 * (range.max - range.min) * 0.2;
        newVal = Math.max(range.min, Math.min(range.max, Math.round(newVal / range.step) * range.step));
...
async function runUltraHyperoptimizer() {
  ...
  logger.log(`Configuration: ${CONFIG.TOTAL_ITERATIONS.toLocaleString()} iterations, ${CONFIG.POPULATION_SIZE} pop, ${CONFIG.NUM_ISLANDS} islands, ${ALL_SYMBOLS.length} symbols`);

  logger.setPhase('Data Loading');
  const bars = await loadMarketData(ALL_SYMBOLS, CONFIG.YEARS_OF_DATA);
  if (bars.size < 10) { logger.log('❌ Insufficient data. Aborting.'); return; }

  const tradingSymbols = Array.from(bars.keys());
  const refBars = bars.get('SPY') || bars.values().next().value;
  const startIdx = 60, endIdx = refBars.length;
  logger.log(`Trading: ${tradingSymbols.length} symbols, ${endIdx - startIdx} days`);
  ...
  for (let i = 0; i < CONFIG.NUM_ISLANDS; i++) {
    ...
    for (let j = 0; j < CONFIG.POPULATION_SIZE / CONFIG.NUM_ISLANDS; j++) pop.push(generateRandomGenome(0, i));
    islands.push(pop);
  }

  const state: OptimizationState = { generation: 0, totalEvaluations: 0, globalBest: null, globalBestResult: null, islands, startTime: new Date() };
  const maxGenerations = Math.ceil(CONFIG.TOTAL_ITERATIONS / CONFIG.POPULATION_SIZE);
  ...
  for (let generation = 0; generation < maxGenerations; generation++) {
    ...
    for (let islandIdx = 0; islandIdx < CONFIG.NUM_ISLANDS; islandIdx++) {
        ...
        await Promise.all(batch.map(async (genome) => {
          if (genome.fitness === 0) {
            ...
            try {
              ...
              genome.sharpe = result.sharpe; genome.sortino = result.sortino; genome.calmar = result.calmar;
              genome.winRate = result.winRate; genome.totalReturn = result.totalReturn; genome.maxDrawdown = result.maxDrawdown;
              genome.trades = result.trades; genome.evaluationTime = Date.now() - evalStart;
              genome.regime = detectMarketRegime((bars.get('SPY') || []).slice(0, endIdx).map(b => b.c), genome.genes.regimeLookback || 50);
              state.totalEvaluations++;

              if (!state.globalBest || genome.fitness > state.globalBest.fitness) {
                const evaluation = judgeSystem.evaluate(genome, result);
                if (evaluation.verdict !== 'SUSPICIOUS') {
                  state.globalBest = { ...genome }; state.globalBestResult = result;
                  logger.logBest(genome, result);
                  evaluation.warnings.forEach(w => logger.log(`   ⚠️ ${w}`));
                }
              }
      ...
      for (let i = 0; i < Math.ceil(CONFIG.ELITE_COUNT / CONFIG.NUM_ISLANDS) && i < island.length; i++) newPop.push({ ...island[i], generation: generation + 1 });
      ...
      while (newPop.length < island.length) {
        if (Math.random() < CONFIG.CROSSOVER_RATE) {
          const p1 = tournamentSelect(island, CONFIG.TOURNAMENT_SIZE) as ProductionGenome;
          const p2 = tournamentSelect(island, CONFIG.TOURNAMENT_SIZE) as ProductionGenome;
          let child = crossover(p1, p2, generation + 1, islandIdx);
          if (Math.random() < mutationRate) child = mutate(child, mutationRate, learningEngine);
          newPop.push(child);
        } else {
          const parent = tournamentSelect(island, CONFIG.TOURNAMENT_SIZE) as ProductionGenome;
    ...
    if (generation > 0 && generation % CONFIG.MIGRATION_INTERVAL === 0) {
      for (let i = 0; i < CONFIG.NUM_ISLANDS; i++) {
        ...
        for (let j = 0; j < CONFIG.MIGRATION_COUNT && j < islands[i].length; j++) target.push({ ...islands[i][j], island: (i + 1) % CONFIG.NUM_ISLANDS, fitness: 0 });
    ...
    if (generation % CONFIG.PROGRESS_REPORT_INTERVAL === 0) {
      logger.logProgress(generation + 1, maxGenerations, { bestFitness: Math.max(...allFitness), avgFitness: allFitness.reduce((a, b) => a + b, 0) / allFitness.length, evalRate: state.totalEvaluations / ((Date.now() - state.startTime.getTime()) / 1000) });
      insights.slice(-3).forEach(i => logger.logInsight(i));
    }

    if (generation > 0 && generation % CONFIG.CHECKPOINT_INTERVAL === 0) saveCheckpoint(state);
    if (learningEngine.isConverged() && generation > 100) { logger.log(`✅ Converged at generation ${generation}`); break; }
  }
  ...
  if (state.globalBest && state.globalBestResult) {
    ...
    logger.log(`   Trades: ${state.globalBestResult.trades}, WinRate: ${(state.globalBestResult.winRate * 100).toFixed(1)}%, PF: ${state.globalBestResult.profitFactor.toFixed(2)}`);
  }
...
runUltraHyperoptimizer().catch(err => { logger.log(`❌ Error: ${err.message}`); logger.flush(); console.error(err); });
