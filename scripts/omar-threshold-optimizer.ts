...
async function runThresholdOptimizer() {
  ...
  console.log('  OMAR THRESHOLD OPTIMIZER - Entry & Exit Threshold Exploration');
  console.log('═'.repeat(80));
  ...
  for (let i = 0; i < bySharpe.length; i++) {
    ...
    console.log(`     Trades: ${r.trades.length} | Profit Factor: ${r.profitFactor.toFixed(2)}`);
  ...
  for (let i = 0; i < bySortino.length; i++) {
    const r = bySortino[i];
    console.log(`  ${i + 1}. Sortino: ${r.sortino.toFixed(3)} | buyThresh=${r.config.buyThreshold.toFixed(2)}, conf=${r.config.confidenceMin.toFixed(2)}, stop=${r.config.atrMultStop.toFixed(2)}x, target=${r.config.atrMultTarget.toFixed(2)}x`);
  }
  ...
  for (let i = 0; i < byCalmar.length; i++) {
    const r = byCalmar[i];
    console.log(`  ${i + 1}. Calmar: ${r.calmar.toFixed(3)} | buyThresh=${r.config.buyThreshold.toFixed(2)}, conf=${r.config.confidenceMin.toFixed(2)}, stop=${r.config.atrMultStop.toFixed(2)}x, target=${r.config.atrMultTarget.toFixed(2)}x`);
  }
  ...
  for (let i = 0; i < byReturn.length; i++) {
    const r = byReturn[i];
    console.log(`  ${i + 1}. Return: ${(r.totalReturn * 100).toFixed(2)}% | buyThresh=${r.config.buyThreshold.toFixed(2)}, conf=${r.config.confidenceMin.toFixed(2)}, stop=${r.config.atrMultStop.toFixed(2)}x, target=${r.config.atrMultTarget.toFixed(2)}x`);
  }
  ...
  for (let i = 0; i < byWinRate.length; i++) {
    const r = byWinRate[i];
    console.log(`  ${i + 1}. Win Rate: ${(r.winRate * 100).toFixed(1)}% | buyThresh=${r.config.buyThreshold.toFixed(2)}, conf=${r.config.confidenceMin.toFixed(2)}, stop=${r.config.atrMultStop.toFixed(2)}x, target=${r.config.atrMultTarget.toFixed(2)}x`);
  }
  ...
  for (let i = 0; i < byProfitFactor.length; i++) {
    ...
    console.log(`  ${i + 1}. Profit Factor: ${r.profitFactor.toFixed(2)} | buyThresh=${r.config.buyThreshold.toFixed(2)}, conf=${r.config.confidenceMin.toFixed(2)}, stop=${r.config.atrMultStop.toFixed(2)}x, target=${r.config.atrMultTarget.toFixed(2)}x`);
  }
  ...
  console.log(`    Avg: ${(riskRewards.reduce((a, b) => a + b, 0) / riskRewards.length).toFixed(2)}:1`);
