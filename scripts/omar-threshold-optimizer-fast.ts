#!/usr/bin/env npx tsx
/**
 * OMAR THRESHOLD OPTIMIZER (FAST VERSION) - Entry & Exit Threshold Exploration
 *
 ...
 *
 * Optimized to test ~1200 combinations in reasonable time
 */
...
// Threshold optimization ranges (coarser steps for faster execution)
const BUY_THRESHOLD_MIN = 0.05;
const BUY_THRESHOLD_MAX = 0.25;
const BUY_THRESHOLD_STEP = 0.05;

const CONFIDENCE_MIN = 0.15;
const CONFIDENCE_MAX = 0.45;
const CONFIDENCE_STEP = 0.06;

const ATR_STOP_MIN = 0.5;
const ATR_STOP_MAX = 3.0;
const ATR_STOP_STEP = 0.5;

const ATR_TARGET_MIN = 1.5;
const ATR_TARGET_MAX = 8.0;
const ATR_TARGET_STEP = 1.0;

// Symbol universe (reduced for speed)
const SYMBOLS = [
...
async function runThresholdOptimizer() {
  ...
  console.log('  OMAR THRESHOLD OPTIMIZER (FAST) - Entry & Exit Threshold Exploration');
  console.log('═'.repeat(80));
      ...
      for (let atrMultStop = ATR_STOP_MIN; atrMultStop <= ATR_STOP_MAX; atrMultStop += ATR_STOP_STEP) {
        for (let atrMultTarget = ATR_TARGET_MIN; atrMultTarget <= ATR_TARGET_MAX; atrMultTarget += ATR_TARGET_STEP) {
          ...
          try {
            ...
            if (iteration % 25 === 0 || iteration === totalIterations) {
  ...
  const bySortino = [...results].sort((a, b) => b.sortino - a.sortino).slice(0, 5);
  ...
  const byProfitFactor = [...results].sort((a, b) => b.profitFactor - a.profitFactor).slice(0, 5);
  ...
  for (let i = 0; i < bySharpe.length; i++) {
    ...
    console.log(`     Return: ${(r.totalReturn * 100).toFixed(2)}% | MaxDD: ${(r.maxDrawdown * 100).toFixed(2)}% | Win Rate: ${(r.winRate * 100).toFixed(1)}%`);
    console.log(`     Trades: ${r.trades.length} | Profit Factor: ${r.profitFactor.toFixed(2)} | Stops: ${r.stopsHit} | Targets: ${r.targetsHit}`);
    console.log(`     Avg Win: $${r.avgWin.toFixed(2)} | Avg Loss: $${r.avgLoss.toFixed(2)}`);
    console.log(`     Config: buyThresh=${r.config.buyThreshold.toFixed(2)}, conf=${r.config.confidenceMin.toFixed(2)}, stop=${r.config.atrMultStop.toFixed(2)}x, target=${r.config.atrMultTarget.toFixed(2)}x`);
  ...
  for (let i = 0; i < bySortino.length; i++) {
    const r = bySortino[i];
    console.log(`  ${i + 1}. Sortino: ${r.sortino.toFixed(3)} | Config: buyThresh=${r.config.buyThreshold.toFixed(2)}, conf=${r.config.confidenceMin.toFixed(2)}, stop=${r.config.atrMultStop.toFixed(2)}x, target=${r.config.atrMultTarget.toFixed(2)}x`);
  }
  ...
  for (let i = 0; i < byCalmar.length; i++) {
    const r = byCalmar[i];
    console.log(`  ${i + 1}. Calmar: ${r.calmar.toFixed(3)} | Config: buyThresh=${r.config.buyThreshold.toFixed(2)}, conf=${r.config.confidenceMin.toFixed(2)}, stop=${r.config.atrMultStop.toFixed(2)}x, target=${r.config.atrMultTarget.toFixed(2)}x`);
  }
  ...
  for (let i = 0; i < byReturn.length; i++) {
    const r = byReturn[i];
    console.log(`  ${i + 1}. Return: ${(r.totalReturn * 100).toFixed(2)}% | Config: buyThresh=${r.config.buyThreshold.toFixed(2)}, conf=${r.config.confidenceMin.toFixed(2)}, stop=${r.config.atrMultStop.toFixed(2)}x, target=${r.config.atrMultTarget.toFixed(2)}x`);
  }
  ...
  const topPerformers = [...results].sort((a, b) => b.sharpe - a.sharpe).slice(0, Math.ceil(results.length * 0.2));
  ...
  console.log(`\n  Buy Threshold (${topPerformers.length} top configs):`);
  console.log(`    Min: ${Math.min(...buyThresholds).toFixed(2)} | Max: ${Math.max(...buyThresholds).toFixed(2)} | Avg: ${(buyThresholds.reduce((a, b) => a + b, 0) / buyThresholds.length).toFixed(2)}`);

  console.log(`\n  Confidence Min:`);
  console.log(`    Min: ${Math.min(...confidenceMins).toFixed(2)} | Max: ${Math.max(...confidenceMins).toFixed(2)} | Avg: ${(confidenceMins.reduce((a, b) => a + b, 0) / confidenceMins.length).toFixed(2)}`);

  console.log(`\n  ATR Stop Multiplier:`);
  console.log(`    Min: ${Math.min(...atrStops).toFixed(2)} | Max: ${Math.max(...atrStops).toFixed(2)} | Avg: ${(atrStops.reduce((a, b) => a + b, 0) / atrStops.length).toFixed(2)}`);

  console.log(`\n  ATR Target Multiplier:`);
  console.log(`    Min: ${Math.min(...atrTargets).toFixed(2)} | Max: ${Math.max(...atrTargets).toFixed(2)} | Avg: ${(atrTargets.reduce((a, b) => a + b, 0) / atrTargets.length).toFixed(2)}`);

  console.log(`\n  Risk/Reward Ratios:`);
  const riskRewards = topPerformers.map(r => r.riskRewardRatio);
  console.log(`    Min: ${Math.min(...riskRewards).toFixed(2)}:1 | Max: ${Math.max(...riskRewards).toFixed(2)}:1 | Avg: ${(riskRewards.reduce((a, b) => a + b, 0) / riskRewards.length).toFixed(2)}:1`);
  ...
  console.log(`    Stops Hit: ${best.stopsHit} | Targets Hit: ${best.targetsHit}`);
