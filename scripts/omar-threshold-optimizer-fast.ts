...
async function runThresholdOptimizer() {
  ...
  console.log(
    "  OMAR THRESHOLD OPTIMIZER (FAST) - Entry & Exit Threshold Exploration"
  );
  console.log("═".repeat(80));
  ...
  for (let i = 0; i < bySharpe.length; i++) {
    ...
    console.log(
      `     Trades: ${r.trades.length} | Profit Factor: ${r.profitFactor.toFixed(2)} | Stops: ${r.stopsHit} | Targets: ${r.targetsHit}`
    );
    console.log(
  ...
  for (let i = 0; i < bySortino.length; i++) {
    const r = bySortino[i];
    console.log(
      `  ${i + 1}. Sortino: ${r.sortino.toFixed(3)} | Config: buyThresh=${r.config.buyThreshold.toFixed(2)}, conf=${r.config.confidenceMin.toFixed(2)}, stop=${r.config.atrMultStop.toFixed(2)}x, target=${r.config.atrMultTarget.toFixed(2)}x`
    );
  }
  ...
  for (let i = 0; i < byCalmar.length; i++) {
    const r = byCalmar[i];
    console.log(
      `  ${i + 1}. Calmar: ${r.calmar.toFixed(3)} | Config: buyThresh=${r.config.buyThreshold.toFixed(2)}, conf=${r.config.confidenceMin.toFixed(2)}, stop=${r.config.atrMultStop.toFixed(2)}x, target=${r.config.atrMultTarget.toFixed(2)}x`
    );
  }
  ...
  for (let i = 0; i < byReturn.length; i++) {
    const r = byReturn[i];
    console.log(
      `  ${i + 1}. Return: ${(r.totalReturn * 100).toFixed(2)}% | Config: buyThresh=${r.config.buyThreshold.toFixed(2)}, conf=${r.config.confidenceMin.toFixed(2)}, stop=${r.config.atrMultStop.toFixed(2)}x, target=${r.config.atrMultTarget.toFixed(2)}x`
    );
  }
  ...
  console.log(
    `    Min: ${Math.min(...buyThresholds).toFixed(2)} | Max: ${Math.max(...buyThresholds).toFixed(2)} | Avg: ${(buyThresholds.reduce((a, b) => a + b, 0) / buyThresholds.length).toFixed(2)}`
  );

  console.log(`\n  Confidence Min:`);
  console.log(
    `    Min: ${Math.min(...confidenceMins).toFixed(2)} | Max: ${Math.max(...confidenceMins).toFixed(2)} | Avg: ${(confidenceMins.reduce((a, b) => a + b, 0) / confidenceMins.length).toFixed(2)}`
  );

  console.log(`\n  ATR Stop Multiplier:`);
  console.log(
    `    Min: ${Math.min(...atrStops).toFixed(2)} | Max: ${Math.max(...atrStops).toFixed(2)} | Avg: ${(atrStops.reduce((a, b) => a + b, 0) / atrStops.length).toFixed(2)}`
  );

  console.log(`\n  ATR Target Multiplier:`);
  console.log(
    `    Min: ${Math.min(...atrTargets).toFixed(2)} | Max: ${Math.max(...atrTargets).toFixed(2)} | Avg: ${(atrTargets.reduce((a, b) => a + b, 0) / atrTargets.length).toFixed(2)}`
  );

  console.log(`\n  Risk/Reward Ratios:`);
  const riskRewards = topPerformers.map((r) => r.riskRewardRatio);
  console.log(
    `    Min: ${Math.min(...riskRewards).toFixed(2)}:1 | Max: ${Math.max(...riskRewards).toFixed(2)}:1 | Avg: ${(riskRewards.reduce((a, b) => a + b, 0) / riskRewards.length).toFixed(2)}:1`
  );
  ...
  console.log(
    `    Avg Win: $${best.avgWin.toFixed(2)} | Avg Loss: $${best.avgLoss.toFixed(2)}`
  );
  console.log(
    `    Stops Hit: ${best.stopsHit} | Targets Hit: ${best.targetsHit}`
  );
