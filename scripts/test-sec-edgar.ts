...
async function testSecEdgar() {
  ...
  try {
    ...
    if (fundamentals) {
      ...
      console.log(`  Total Liabilities: ${fundamentals.totalLiabilities ? '$' + (fundamentals.totalLiabilities / 1e9).toFixed(2) + 'B' : 'N/A'}`);
      console.log(`  EPS: ${fundamentals.eps ? '$' + fundamentals.eps.toFixed(2) : 'N/A'}`);
      console.log(`  Shares Outstanding: ${fundamentals.sharesOutstanding ? (fundamentals.sharesOutstanding / 1e9).toFixed(2) + 'B' : 'N/A'}`);
    } else {
    ...
    console.log('-'.repeat(80));
    const transactions = await secEdgarConnector.getInsiderTransactions(TEST_TICKER, 10);
    if (transactions && transactions.length > 0) {
      ...
      transactions.slice(0, 5).forEach((tx, idx) => {
        ...
        console.log(`  Filing Date: ${tx.filingDate.toISOString().split('T')[0]}`);
        ...
        console.log(`  Value: ${tx.value ? '$' + tx.value.toLocaleString() : 'N/A'}`);
        console.log(`  Shares Owned After: ${tx.sharesOwnedAfter.toLocaleString()}`);
        console.log(`  Ownership: ${tx.isDirectOwnership ? 'Direct' : 'Indirect'}`);
      ...
      console.log('✗ No insider transactions found (this may be normal if there are no recent Form 4 filings)');
    }
    console.log();
    ...
    console.log('-'.repeat(80));
    const insiderSummary = await secEdgarConnector.getInsiderSummary(TEST_TICKER, 90);
    if (insiderSummary) {
      console.log('✓ Insider summary retrieved:');
      console.log(`  Company: ${insiderSummary.companyName} (${insiderSummary.ticker})`);
      console.log(`  CIK: ${insiderSummary.cik}`);
      console.log(`  Total Insider Buys: ${insiderSummary.totalInsiderBuys.toLocaleString()} shares`);
      console.log(`  Total Insider Sells: ${insiderSummary.totalInsiderSells.toLocaleString()} shares`);
      console.log(`  Net Activity: ${insiderSummary.netInsiderActivity.toLocaleString()} shares`);
      console.log(`  Net Value: $${insiderSummary.netInsiderValue.toLocaleString()}`);
      console.log(`  Buy-to-Sell Ratio: ${insiderSummary.buyToSellRatio === Infinity ? '∞' : insiderSummary.buyToSellRatio.toFixed(2)}`);
      console.log(`  Sentiment: ${insiderSummary.sentiment.toUpperCase()}`);
      console.log(`  Recent Transactions Count: ${insiderSummary.recentTransactions.length}`);
      console.log(`  Last Updated: ${insiderSummary.lastUpdated.toISOString()}`);
      console.log();

      if (insiderSummary.recentTransactions.length > 0) {
        console.log('  Recent Transactions:');
        insiderSummary.recentTransactions.slice(0, 3).forEach((tx, idx) => {
          console.log(`    ${idx + 1}. ${tx.reportingOwner} (${tx.relationship})`);
          console.log(`       ${tx.transactionType === 'P' ? 'BOUGHT' : tx.transactionType === 'S' ? 'SOLD' : tx.transactionType} ${tx.sharesTransacted.toLocaleString()} shares`);
    ...
    console.log('-'.repeat(80));
    const filings = await secEdgarConnector.getRecentFilings(TEST_TICKER, ['10-K', '10-Q', '8-K'], 5);
    if (filings && filings.length > 0) {
      console.log(`✓ Found ${filings.length} recent filings:`);
      filings.forEach((filing, idx) => {
        console.log(`  ${idx + 1}. Form ${filing.form} - Filed: ${filing.filingDate.toISOString().split('T')[0]}`);
    ...
    console.log(`✓ Insider Transactions: ${transactions && transactions.length > 0 ? 'PASSED (' + transactions.length + ' found)' : 'PASSED (0 found)'}`);
    console.log(`✓ Insider Summary: ${insiderSummary ? 'PASSED' : 'FAILED'}`);
    console.log(`✓ Recent Filings: ${filings && filings.length > 0 ? 'PASSED (' + filings.length + ' found)' : 'FAILED'}`);
    console.log();

    const allPassed = cik && companyInfo && fundamentals && insiderSummary && filings && filings.length > 0;
    console.log(`Overall: ${allPassed ? '✓ ALL TESTS PASSED' : '⚠ SOME TESTS HAD ISSUES'}`);
    console.log('='.repeat(80));
