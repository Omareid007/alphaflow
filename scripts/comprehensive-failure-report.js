...
function categorizeError(error, payload) {
  ...
  if (lowerError.includes('market orders not allowed') &&
  ...
  // Price validation
  if (lowerError.includes('price') || lowerError.includes('limit_price') ||
  ...
  // Symbol/asset errors
  if (lowerError.includes('symbol') || lowerError.includes('asset') ||
      lowerError.includes('not tradable') || lowerError.includes('not found')) {
    return {
...
async function generateReport() {
  ...
  try {
    ...
    failedWorkItems.forEach(wi => {
      ...
      else if (hour >= 13 && hour < 21) timeSlot = '09:00-17:00 ET (Regular Hours)';
      else if (hour >= 21 || hour < 1) timeSlot = '17:00-20:00 ET (After-Hours)';
    ...
    sortedCategories.forEach((cat, idx) => {
      ...
      cat.examples.forEach((ex, i) => {
        ...
        console.log(`│      Extended Hours: ${ex.extendedHours}, Attempts: ${ex.attempts}`);
        console.log(`│      Time: ${new Date(ex.createdAt).toISOString()}`);
        console.log(`│      Error: ${ex.error?.substring(0, 100)}${ex.error?.length > 100 ? '...' : ''}`);
    ...
    console.log('\n┌─────────────┬───────┬─────────────────────────────────────────────────┐');
    console.log('│ Symbol      │ Count │ Failure Categories                              │');
    console.log('├─────────────┼───────┼─────────────────────────────────────────────────┤');
    sortedSymbols.forEach(([symbol, data]) => {
      const categories = Array.from(data.categories).join(', ');
      const truncCat = categories.length > 45 ? categories.substring(0, 42) + '...' : categories;
      console.log(`│ ${symbol.padEnd(11)} │ ${String(data.count).padStart(5)} │ ${truncCat.padEnd(47)} │`);
    });
    console.log('└─────────────┴───────┴─────────────────────────────────────────────────┘');
    ...
    const sortedTimes = Array.from(timeOfDayStats.entries())
      .sort((a, b) => b[1].count - a[1].count);
    ...
    const sortedOrderTypes = Array.from(orderTypeStats.entries())
      .sort((a, b) => b[1].count - a[1].count);

    console.log('\n┌─────────────────────┬───────┬───────────────────────────────────────┐');
    console.log('│ Type/TIF            │ Count │ Failure Categories                    │');
    console.log('├─────────────────────┼───────┼───────────────────────────────────────┤');
    sortedOrderTypes.forEach(([type, data]) => {
      const categories = Array.from(data.categories).join(', ');
      const truncCat = categories.length > 37 ? categories.substring(0, 34) + '...' : categories;
      console.log(`│ ${type.padEnd(19)} │ ${String(data.count).padStart(5)} │ ${truncCat.padEnd(37)} │`);
    });
    console.log('└─────────────────────┴───────┴───────────────────────────────────────┘');
    ...
    console.log(`Critical/High Severity:      ${sortedCategories.filter(c => c.severity === 'CRITICAL' || c.severity === 'HIGH').length} categories`);
    ...
    const totalAttempts = successCount + failedWorkItems.filter(w => w.type === 'ORDER_SUBMIT').length;
...
function getCodeLocation(category) {
  const locations = {
    'MARKET_ORDER_EXTENDED_HOURS': 'server/trading/unified-order-executor.ts or services/trading-engine/order-manager.ts',
    ...
    'SYSTEM_ERROR': 'Check stack trace - likely server/trading/* or services/trading-engine/*',
