...
class PerformanceTester {
  ...
  private async runTest(
    ...
    if (results[0]?.payloadSize) {
      console.log(`  Payload: ${(results[0].payloadSize / 1024).toFixed(2)} KB`);
    }
  }
  ...
  private async loadTest(
    ...
    const durations = results.map(r => r.duration);
    const errors = results.filter(r => !r.success).length;
    const successRate = ((results.length - errors) / results.length * 100).toFixed(2);
    const throughput = (results.length / (totalTime / 1000)).toFixed(2);

    console.log(`  Total Time: ${totalTime.toFixed(2)}ms`);
    console.log(`  Avg Response: ${(durations.reduce((a, b) => a + b, 0) / durations.length).toFixed(2)}ms`);
  ...
  async runAllTests() {
    ...
    await this.runTest('ai_decisions_enriched', 'GET', '/api/ai-decisions/enriched', 5);
    await this.runTest('activity_timeline', 'GET', '/api/activity/timeline', 10);
    await this.runTest('analytics_summary', 'GET', '/api/analytics/summary', 10);

    // Admin/Monitoring Endpoints
    console.log('\n\n📋 CATEGORY: Admin & Monitoring');
    console.log('─'.repeat(60));
    await this.runTest('connectors_status', 'GET', '/api/connectors/status', 10);
  ...
  private generateReport() {
    ...
    if (withErrors.length === 0) {
      ...
      withErrors.forEach(m => {
        ...
        console.log(`❌ ${m.endpoint}: ${m.errors}/${m.requests} (${errorRate}%)`);
      });
    }
