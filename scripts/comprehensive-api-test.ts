...
function extractEndpointsFromFile(filePath: string): EndpointInfo[] {
  ...
  const middlewarePattern = /(authMiddleware|adminTokenMiddleware|requireCapability)/;
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (routeMatch) {
      ...
      if (line.includes('adminTokenMiddleware')) middlewares.push('adminTokenMiddleware');
      if (line.includes('requireCapability')) {
        const capMatch = line.match(/requireCapability\s*\(\s*["']([^"']+)["']/);
        if (capMatch) {
      ...
      if (!validationPresent && (method === 'POST' || method === 'PUT' || method === 'PATCH')) {
...
function checkValidation(content: string, lineIndex: number, lines: string[]): boolean {
...
function categorizeEndpoint(path: string): string {
  ...
  if (path.includes('/autonomous/') || path.includes('/agent/')) return 'Autonomous Trading';
  if (path.includes('/strategies')) return 'Strategies';
  if (path.includes('/positions') || path.includes('/orders') || path.includes('/trades')) return 'Trading';
  if (path.includes('/alpaca/')) return 'Alpaca Integration';
  if (path.includes('/ai/')) return 'AI';
  if (path.includes('/crypto/') || path.includes('/stock/')) return 'Market Data';
  if (path.includes('/backtests')) return 'Backtesting';
  if (path.includes('/health') || path.includes('/status')) return 'Health';
  if (path.includes('/webhooks') || path.includes('/notifications')) return 'Notifications';
  return 'Other';
}

function extractRouterEndpoints(routerPath: string, mountPath: string): EndpointInfo[] {
...
async function runComprehensiveTest(): Promise<TestReport> {
  ...
  if (authCoverage.percentage < 80) {
    recommendations.push('Consider adding authentication to more endpoints for better security');
  }
  ...
  const publicTradingEndpoints = publicEndpoints.filter(e =>
    e.path.includes('/orders') ||
    e.path.includes('/positions') ||
    e.path.includes('/trades')
  );
  if (publicTradingEndpoints.length > 0) {
    recommendations.push('URGENT: Trading endpoints should require authentication');
...
function printReport(report: TestReport) {
  ...
  console.log(`Protected (Auth Required): ${report.protectedEndpoints} (${report.authCoverage.percentage.toFixed(1)}%)`);
  console.log(`Public (No Auth):          ${report.publicEndpoints} (${(100 - report.authCoverage.percentage).toFixed(1)}%)`);
  console.log('');
  ...
  for (const category of categories) {
    ...
    for (const endpoint of endpoints) {
      ...
      console.log(`  ${auth} ${endpoint.method.padEnd(6)} ${endpoint.path}${issues}`);
    }
  }
  ...
  console.log(`Coverage percentage:  ${report.authCoverage.percentage.toFixed(1)}%`);
...
(async () => {
  try {
    ...
    // Save report to file
    const reportPath = path.join(process.cwd(), 'API_ENDPOINT_SECURITY_REPORT.md');
...
function generateMarkdownReport(report: TestReport): string {
  ...
  for (const category of categories) {
    ...
    for (const endpoint of endpoints) {
      ...
      const issues = endpoint.issues.length > 0 ? endpoint.issues.join('; ') : '-';
