...
function parseOpenSpec(filePath: string): Capability {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('# ') && !capability.name) {
      capability.name = line.replace('# ', '').replace(' Capability', '').trim();
    }
...
function scenarioToTestCase(scenario: Scenario, capability: Capability): TestCase {
...
function detectEndpoint(scenario: Scenario, capability: Capability): ApiEndpoint | null {
...
function generateTestFile(capability: Capability, testCases: TestCase[]): string {
...
function generateRequirementSuite(requirement: Requirement, allTestCases: TestCase[]): string {
