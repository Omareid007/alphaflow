...
const SYMBOLS = [
  ...
  'AMD', 'JPM', 'BAC', 'V', 'MA', 'UNH', 'JNJ', 'WMT', 'COST',
  'SPY', 'QQQ', 'IWM', 'XLF', 'XLK',
];
...
function calculateMACD(closes: number[], fast: number, slow: number, signal: number): { macd: (number | null)[], signal: (number | null)[], histogram: (number | null)[] } {
  ...
  for (let i = 0; i < macdLine.length; i++) {
    if (macdLine[i] === null) histogram.push(null);
      ...
      histogram.push(signalLine[si] !== null ? macdLine[i]! - signalLine[si]! : null);
      si++;
    }
  }
...
function calculateBollingerBands(closes: number[], period: number, stdDev: number): { upper: (number | null)[], middle: (number | null)[], lower: (number | null)[] } {
...
function calculateStochastic(highs: number[], lows: number[], closes: number[], period: number, smoothK: number, smoothD: number): { k: (number | null)[], d: (number | null)[] } {
...
function generateSignal(bars: AlpacaBar[], config: IndicatorConfig): { score: number, confidence: number } {
  ...
  // Stochastic
  const stoch = calculateStochastic(highs, lows, closes, config.stochPeriod, config.stochSmoothK, config.stochSmoothD);
  ...
  // Momentum
  const momentum5 = closes.length >= 6 ? (closes[closes.length - 1] - closes[closes.length - 6]) / closes[closes.length - 6] : 0;
...
async function runBacktest(bars: Map<string, AlpacaBar[]>, config: IndicatorConfig): Promise<BacktestResult> {
  ...
  for (let dayIdx = 50; dayIdx < tradingDays.length; dayIdx++) {
    ...
    if (positions.size < FIXED_CONFIG.maxPositions) {
      ...
      for (const [symbol, symbolBars] of bars) {
        ...
        if (signal.score >= FIXED_CONFIG.buyThreshold && signal.confidence >= FIXED_CONFIG.confidenceMin) {
      ...
      for (const candidate of candidates.slice(0, FIXED_CONFIG.maxPositions - positions.size)) {
        ...
        if (shares > 0 && shares * candidate.price <= capital) {
          const stopLoss = candidate.price - candidate.atr * FIXED_CONFIG.atrMultStop;
          const takeProfit = candidate.price + candidate.atr * FIXED_CONFIG.atrMultTarget;
    ...
    if (equityCurve.length > 1) {
      dailyReturns.push((currentEquity - equityCurve[equityCurve.length - 2]) / equityCurve[equityCurve.length - 2]);
    }
  ...
  // Calculate metrics
  const totalReturn = (equityCurve[equityCurve.length - 1] - FIXED_CONFIG.initialCapital) / FIXED_CONFIG.initialCapital;
  ...
  const stdReturn = dailyReturns.length > 1 ? Math.sqrt(dailyReturns.reduce((sum, r) => sum + Math.pow(r - avgReturn, 2), 0) / dailyReturns.length) : 0.0001;
  const sharpe = stdReturn > 0 ? (avgReturn * 252) / (stdReturn * Math.sqrt(252)) : 0;

  const negativeReturns = dailyReturns.filter(r => r < 0);
  const downstdReturn = negativeReturns.length > 1 ? Math.sqrt(negativeReturns.reduce((sum, r) => sum + r * r, 0) / negativeReturns.length) : 0.0001;
...
async function main() {
  ...
  // Process in batches
  for (let batchStart = 0; batchStart < configs.length; batchStart += BATCH_SIZE) {
    ...
    for (const config of batch) {
      ...
      try {
        ...
        if (!bestResult || score > bestResult.score) {
          ...
          console.log(`  ├─ Sharpe: ${metrics.sharpe.toFixed(3)} | Sortino: ${metrics.sortino.toFixed(3)} | Calmar: ${metrics.calmar.toFixed(3)}`);
  ...
  if (bestResult) {
    ...
    for (let i = 0; i < sortedResults.length; i++) {
      ...
      console.log(`  ├─ Score: ${r.score.toFixed(2)} | Sharpe: ${r.metrics.sharpe.toFixed(2)} | Return: ${(r.metrics.totalReturn * 100).toFixed(1)}%`);
      console.log(`  └─ RSI: ${r.config.rsiPeriod}/${r.config.rsiOversold}/${r.config.rsiOverbought} | MACD: ${r.config.macdFast}/${r.config.macdSlow}/${r.config.macdSignal} | BB: ${r.config.bbPeriod}/${r.config.bbStdDev.toFixed(1)}`);
    }
    ...
    const paramStats: Record<string, { avg: number, min: number, max: number }> = {};
    ...
    console.log(`  ├─ RSI Period: ${paramStats.rsiPeriod.min}-${paramStats.rsiPeriod.max} (avg: ${paramStats.rsiPeriod.avg.toFixed(1)})`);
    console.log(`  ├─ RSI Oversold: ${paramStats.rsiOversold.min}-${paramStats.rsiOversold.max} (avg: ${paramStats.rsiOversold.avg.toFixed(1)})`);
    console.log(`  ├─ RSI Overbought: ${paramStats.rsiOverbought.min}-${paramStats.rsiOverbought.max} (avg: ${paramStats.rsiOverbought.avg.toFixed(1)})`);
    console.log(`  ├─ MACD Fast: ${paramStats.macdFast.min}-${paramStats.macdFast.max} (avg: ${paramStats.macdFast.avg.toFixed(1)})`);
    console.log(`  ├─ MACD Slow: ${paramStats.macdSlow.min}-${paramStats.macdSlow.max} (avg: ${paramStats.macdSlow.avg.toFixed(1)})`);
    console.log(`  ├─ MACD Signal: ${paramStats.macdSignal.min}-${paramStats.macdSignal.max} (avg: ${paramStats.macdSignal.avg.toFixed(1)})`);
    console.log(`  ├─ BB Period: ${paramStats.bbPeriod.min}-${paramStats.bbPeriod.max} (avg: ${paramStats.bbPeriod.avg.toFixed(1)})`);
    console.log(`  ├─ BB StdDev: ${paramStats.bbStdDev.min.toFixed(1)}-${paramStats.bbStdDev.max.toFixed(1)} (avg: ${paramStats.bbStdDev.avg.toFixed(1)})`);
    console.log(`  ├─ ATR Period: ${paramStats.atrPeriod.min}-${paramStats.atrPeriod.max} (avg: ${paramStats.atrPeriod.avg.toFixed(1)})`);
    console.log(`  └─ Stoch Period: ${paramStats.stochPeriod.min}-${paramStats.stochPeriod.max} (avg: ${paramStats.stochPeriod.avg.toFixed(1)})`);
