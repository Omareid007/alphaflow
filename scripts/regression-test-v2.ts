...
const client1 = wrapper(axios.create({ jar: jar1, withCredentials: true, baseURL: API_BASE }));
const client2 = wrapper(axios.create({ jar: jar2, withCredentials: true, baseURL: API_BASE }));
...
async function testAuthentication() {
  ...
  try {
    ...
    if (res.status === 200 && res.data.id) {
      ...
      logTest(category, 'Signup User 1', false, 'No user ID in response', res.data, duration);
    }
  } catch (error) {
  ...
  try {
    ...
    if (res.status === 200 && res.data.id) {
      ...
      logTest(category, 'Signup User 2', false, 'No user ID in response', res.data, duration);
    }
  } catch (error) {
  ...
  try {
    ...
    if (res.status === 200 && res.data.id) {
      logTest(category, 'Login with valid credentials', true, undefined, undefined, duration);
    } else {
      logTest(category, 'Login with valid credentials', false, 'Invalid response', res.data, duration);
    }
  } catch (error) {
    logTest(category, 'Login with valid credentials', false, sanitizeErrorMessage(error));
  }
  ...
  try {
    ...
    logTest(category, 'Login with invalid credentials (should fail)', false, 'Should have returned 401');
  } catch (error) {
    if (axios.isAxiosError(error) && error.response?.status === 401) {
      logTest(category, 'Login with invalid credentials (should fail)', true);
    } else {
      logTest(category, 'Login with invalid credentials (should fail)', false, sanitizeErrorMessage(error));
    }
  }
  ...
  try {
    ...
    if (res.status === 200 && res.data.username === testUsers.user1.username) {
      logTest(category, 'Get current user (authenticated)', true, undefined, undefined, duration);
    } else {
      logTest(category, 'Get current user (authenticated)', false, 'Wrong user data', res.data, duration);
    }
  } catch (error) {
    logTest(category, 'Get current user (authenticated)', false, sanitizeErrorMessage(error));
  }

  // Test 6: Get current user without auth (should fail)
  try {
    await noAuthClient.get('/api/auth/me');
    logTest(category, 'Get current user without auth (should fail)', false, 'Should have returned 401');
  } catch (error) {
    if (axios.isAxiosError(error) && error.response?.status === 401) {
      logTest(category, 'Get current user without auth (should fail)', true);
    } else {
      logTest(category, 'Get current user without auth (should fail)', false, sanitizeErrorMessage(error));
    }
  }
  ...
  try {
    ...
    logTest(category, 'Duplicate username (should fail)', false, 'Should have returned 400');
  } catch (error) {
    if (axios.isAxiosError(error) && error.response?.status === 400) {
      logTest(category, 'Duplicate username (should fail)', true);
    } else {
      logTest(category, 'Duplicate username (should fail)', false, sanitizeErrorMessage(error));
    }
  }
}
...
async function testProtectedEndpoints() {
  ...
  for (const endpoint of endpoints) {
    try {
      ...
      logTest(category, `${endpoint} without auth (should fail)`, false, 'Should have returned 401');
    } catch (error) {
      if (axios.isAxiosError(error) && error.response?.status === 401) {
        ...
        if (axios.isAxiosError(error) && error.response?.status === 503) {
          logTest(category, `${endpoint} without auth (should fail)`, true, 'Service unavailable (expected)');
        } else {
          logTest(category, `${endpoint} without auth (should fail)`, false, sanitizeErrorMessage(error));
        }
      }
    }
  }
  ...
  for (const endpoint of authEndpoints) {
    try {
      ...
      if (res.status === 200 && Array.isArray(res.data)) {
        logTest(category, `${endpoint} with auth`, true, undefined, undefined, duration);
      } else {
        logTest(category, `${endpoint} with auth`, false, 'Invalid response', res.data, duration);
      }
    } catch (error) {
      logTest(category, `${endpoint} with auth`, false, sanitizeErrorMessage(error));
    }
  }
}
...
async function testStrategyManagement() {
  ...
  try {
    ...
    if (res.status === 200 && res.data.id) {
      ...
      logTest(category, 'Create strategy', true, undefined, undefined, duration);
    } else {
      logTest(category, 'Create strategy', false, 'No strategy ID', res.data, duration);
    }
  } catch (error) {
  ...
  if (strategyId) {
    try {
      ...
      if (res.status === 200 && res.data.id === strategyId) {
        logTest(category, 'Get strategy by ID', true, undefined, undefined, duration);
      } else {
        logTest(category, 'Get strategy by ID', false, 'Wrong strategy', res.data, duration);
      }
    } catch (error) {
      logTest(category, 'Get strategy by ID', false, sanitizeErrorMessage(error));
    }
  }
  ...
  if (strategyId) {
    try {
      ...
      if (res.status === 200 && res.data.name === 'Updated Test Strategy') {
        logTest(category, 'Update strategy', true, undefined, undefined, duration);
      } else {
        logTest(category, 'Update strategy', false, 'Update failed', res.data, duration);
      }
    } catch (error) {
  ...
  if (strategyId) {
    try {
      ...
      logTest(category, 'User isolation (should fail)', false, 'User 2 should not access User 1 strategy');
    } catch (error) {
      if (axios.isAxiosError(error) && (error.response?.status === 403 || error.response?.status === 404)) {
        ...
        logTest(category, 'User isolation (should fail)', false, sanitizeErrorMessage(error));
      }
    }
  }
  ...
  if (strategyId) {
    try {
      ...
      if (res.status === 200) {
        logTest(category, 'Delete strategy', true, undefined, undefined, duration);
      } else {
        logTest(category, 'Delete strategy', false, 'Delete failed', res.data, duration);
      }
    } catch (error) {
  ...
  try {
    ...
    if (user) {
      ...
      if (strategy && strategy.userId === user.id) {
        ...
        logTest(category, 'createStrategy with userId', false, 'UserId not set');
      }
    }
  } catch (error) {
  ...
  if (strategyId) {
    try {
      ...
      if (user1 && user2) {
        ...
        const user1HasStrategy = user1Strategies.some(s => s.id === strategyId);
        const user2HasStrategy = user2Strategies.some(s => s.id === strategyId);

        if (user1HasStrategy && !user2HasStrategy) {
          logTest(category, 'User data isolation', true);
        } else {
          logTest(category, 'User data isolation', false, `User1: ${user1HasStrategy}, User2: ${user2HasStrategy}`);
        }
      }
    } catch (error) {
      logTest(category, 'User data isolation', false, sanitizeErrorMessage(error));
    }
  }
...
async function testInputSanitization() {
  ...
  try {
    ...
    if (res.data.username && !res.data.username.includes('<script>')) {
      ...
      logTest(category, 'XSS sanitization in username', false, 'Script tags not sanitized', res.data);
    }
  } catch (error) {
    logTest(category, 'XSS sanitization in username', false, sanitizeErrorMessage(error));
  }
  ...
  try {
    ...
    if (res.data && !res.data.name.includes('<img')) {
      ...
      // Cleanup
      if (res.data.id) await client1.delete(`/api/strategies/${res.data.id}`).catch(() => {});
    } else {
      logTest(category, 'XSS sanitization in strategy name', false, 'HTML tags not sanitized', res.data);
    }
  } catch (error) {
    logTest(category, 'XSS sanitization in strategy name', false, sanitizeErrorMessage(error));
  }
  ...
  try {
    ...
    logTest(category, 'SQL injection protection', true, 'ORM protects against SQL injection');
  } catch (error) {
    logTest(category, 'SQL injection protection', true, 'Request rejected (also acceptable)');
  }
}
...
async function testErrorHandling() {
  ...
  try {
    ...
    if (axios.isAxiosError(error) && error.response?.status === 401) {
      ...
      logTest(category, 'Invalid credentials error', false, sanitizeErrorMessage(error));
    }
  }
  ...
  try {
    ...
    logTest(category, 'Missing required fields error', false, 'Should return 400');
  } catch (error) {
    if (axios.isAxiosError(error) && (error.response?.status === 400 || error.response?.status === 422)) {
      ...
      logTest(category, 'Missing required fields error', false, sanitizeErrorMessage(error));
    }
  }
  ...
  try {
    ...
    logTest(category, 'Non-existent resource error', false, 'Should return 404');
  } catch (error) {
    if (axios.isAxiosError(error) && error.response?.status === 404) {
      logTest(category, 'Non-existent resource error', true);
    } else {
      logTest(category, 'Non-existent resource error', false, sanitizeErrorMessage(error));
    }
  }
}
...
async function testPerformance() {
  ...
  try {
    ...
    if (avgTime < 500) {
      logTest(category, 'API response time', true, undefined, `Average: ${avgTime.toFixed(2)}ms`);
    } else {
      logTest(category, 'API response time', false, `Too slow: ${avgTime.toFixed(2)}ms`);
    }
  } catch (error) {
  ...
  try {
    const start = Date.now();
    const promises = Array(10).fill(null).map(() => client1.get('/api/strategies'));
    await Promise.all(promises);
    const duration = Date.now() - start;

    if (duration < 2000) {
      logTest(category, 'Concurrent requests (10 parallel)', true, undefined, `${duration}ms`);
    } else {
      logTest(category, 'Concurrent requests (10 parallel)', false, `Too slow: ${duration}ms`);
    }
  } catch (error) {
    logTest(category, 'Concurrent requests', false, sanitizeErrorMessage(error));
  }
}
