...
async function testEndpoint(
  ...
  try {
    ...
    console.log(`${statusIcon} ${result.passed ? 'Passed' : 'Failed'} - Status: ${result.actualStatus}`);
  } catch (error: any) {
...
function generateReport() {
  ...
  const categories = [
    { name: 'Public Endpoints (No Auth Required)', filter: (r: TestResult) => r.description.includes('(No Auth)') === false && (r.endpoint === '/api/auth/me' || r.endpoint.includes('/trading/candidates') || r.endpoint.includes('/watchlist')) },
    { name: 'Protected Endpoints (Should Return 401 Without Auth)', filter: (r: TestResult) => r.description.includes('(No Auth)') && r.expectedStatus === 401 },
    { name: 'Health Check Endpoints', filter: (r: TestResult) => r.description.includes('Health') },
    { name: 'Additional Endpoints', filter: (r: TestResult) => r.endpoint === '/' || r.endpoint === '/api' || r.endpoint === '/health' },
  ];
  ...
  const authIssues = results.filter(r =>
    r.expectedStatus === 401 && r.actualStatus !== 401
  );
  if (authIssues.length > 0) {
    lines.push('### Authentication Issues');
    lines.push('');
    authIssues.forEach(issue => {
      lines.push(`- **${issue.endpoint}**: Expected 401 but got ${issue.actualStatus}`);
    });
    lines.push('');
  }

  // Response Format Problems
  const formatIssues = results.filter(r =>
    r.passed && r.responseBody.includes('Error')
  );
  if (formatIssues.length > 0) {
    lines.push('### Response Format Issues');
    lines.push('');
    formatIssues.forEach(issue => {
      lines.push(`- **${issue.endpoint}**: Response contains error message despite successful status`);
    });
    lines.push('');
  }
  ...
  const connectionErrors = results.filter(r =>
    r.actualStatus === 'Connection Error'
  );
...
async function main() {
  ...
  await testEndpoint('GET', '/api/strategies', 'List Strategies (No Auth)', 401);
  await testEndpoint('GET', '/api/positions', 'List Positions (No Auth)', 401);
  await testEndpoint('GET', '/api/ai-decisions', 'List AI Decisions (No Auth)', 401);
  ...
  await testEndpoint('GET', '/api/trading/candidates', 'Trading Candidates (Public)', 200);
