...
const MARKET_UNIVERSE = {
  PRECIOUS_METALS: ['GLD', 'IAU', 'SLV', 'PPLT', 'PALL', 'SGOL', 'SIVR', 'OUNZ', 'GLDM', 'BAR'],
  METAL_MINERS: ['NEM', 'GOLD', 'FCX', 'AEM', 'WPM', 'FNV', 'RGLD', 'KGC', 'AU', 'AG', 'PAAS', 'HL', 'CDE', 'EXK', 'MAG', 'SVM', 'SCCO', 'TECK', 'RIO', 'BHP', 'VALE', 'MT', 'NUE', 'STLD', 'CLF', 'X', 'AA', 'CENX'],
  BASE_METALS: ['CPER', 'JJC', 'DBB', 'REMX', 'PICK', 'XME', 'SLX'],
  ENERGY: ['USO', 'BNO', 'UNG', 'BOIL', 'UCO', 'XLE', 'VDE', 'OIH', 'XOP', 'XOM', 'CVX', 'COP', 'SLB', 'EOG', 'MPC', 'PSX', 'VLO', 'OXY', 'DVN', 'HAL', 'BKR', 'FANG', 'PXD'],
  AGRICULTURE: ['DBA', 'CORN', 'WEAT', 'SOYB', 'COW', 'NIB', 'JO', 'SGG', 'BAL', 'MOO', 'VEGI', 'ADM', 'BG', 'DE', 'CTVA', 'NTR', 'MOS', 'CF'],
  BROAD_COMMODITIES: ['DJP', 'GSG', 'PDBC', 'USCI', 'BCI', 'COMT', 'COM'],
  US_LARGE_CAP: ['AAPL', 'MSFT', 'GOOGL', 'AMZN', 'NVDA', 'META', 'TSLA', 'BRK.B', 'UNH', 'XOM', 'JNJ', 'JPM', 'V', 'PG', 'MA', 'HD', 'CVX', 'MRK', 'ABBV', 'LLY', 'PEP', 'KO', 'AVGO', 'COST', 'TMO', 'MCD', 'WMT', 'CSCO', 'ACN', 'ABT', 'BAC', 'PFE', 'CRM', 'LIN', 'DHR', 'ORCL', 'AMD', 'VZ', 'INTC', 'NKE', 'ADBE', 'CMCSA', 'TXN', 'DIS', 'NEE', 'PM', 'WFC', 'BMY', 'RTX', 'UPS'],
  US_MID_CAP: ['PANW', 'FTNT', 'CDNS', 'SNPS', 'ANET', 'CRWD', 'MRVL', 'TEAM', 'DDOG', 'ZS', 'WDAY', 'OKTA', 'SPLK', 'TTD', 'NET', 'DOCU', 'BILL', 'SNOW', 'VEEV', 'HUBS', 'ZI', 'PAYC', 'MNDY', 'CFLT', 'PATH', 'APP', 'S', 'GLBE', 'SMAR', 'GTLB'],
  US_SMALL_CAP: ['SMCI', 'IONQ', 'RKLB', 'JOBY', 'LILM', 'EVGO', 'CHPT', 'BLNK', 'PLUG', 'FCEL', 'BE', 'LCID', 'RIVN', 'FSR', 'NKLA', 'GOEV', 'WKHS', 'RIDE', 'HYLN', 'XOS'],
  SECTOR_ETFS: ['XLF', 'XLK', 'XLE', 'XLV', 'XLI', 'XLP', 'XLY', 'XLB', 'XLU', 'XLRE', 'XLC', 'VGT', 'VHT', 'VFH', 'VCR', 'VDC', 'VIS', 'VAW', 'VPU', 'VOX', 'VNQ'],
  INTERNATIONAL: ['EFA', 'EEM', 'VEU', 'VWO', 'IEFA', 'IEMG', 'FXI', 'EWJ', 'EWG', 'EWU', 'EWZ', 'EWY', 'INDA', 'EWT', 'EWH', 'EWS', 'EWA', 'EWC', 'EWW'],
  BONDS: ['TLT', 'IEF', 'SHY', 'LQD', 'HYG', 'JNK', 'BND', 'AGG', 'TIP', 'EMB', 'MUB', 'VCSH', 'VCIT', 'BNDX'],
  FACTORS: ['MTUM', 'QUAL', 'VLUE', 'SIZE', 'USMV', 'DGRO', 'VIG', 'SCHD', 'DVY', 'HDV'],
  INDICES: ['SPY', 'QQQ', 'IWM', 'DIA', 'MDY', 'IJR', 'VTI', 'VOO', 'IVV', 'RSP'],
...
class ContinuousLearningEngine {
  ...
  getInsights(): LearningInsight[] { return this.insights.slice(-100); }
...
class JudgeSystem {
  ...
  evaluate(genome: UltraGenome, result: BacktestResult): {
    ...
    if (score > 200 && !warnings.some(w => w.includes('OVERFITTING'))) verdict = 'EXCELLENT';
    else if (score > 150) verdict = warnings.some(w => w.includes('OVERFITTING')) ? 'SUSPICIOUS' : 'GOOD';
    else if (score > 100) verdict = 'ACCEPTABLE';
    else verdict = 'POOR';

    this.evaluationHistory.push({ fitness: score, verdict, timestamp: new Date() });
    return { verdict, confidence: Math.min(1, Math.max(0, score / 250)), warnings, suggestions, score };
  }
  ...
  getEvaluationTrend(): { improving: boolean; rate: number } {
    ...
    const avgFirst = recent.slice(0, half).reduce((s, e) => s + e.fitness, 0) / half;
    const avgSecond = recent.slice(half).reduce((s, e) => s + e.fitness, 0) / (recent.length - half);
    return { improving: avgSecond > avgFirst, rate: (avgSecond - avgFirst) / avgFirst };
  }
}
...
function generateSignal(bars: AlpacaBar[], genes: Record<string, number>, idx: number): { score: number; confidence: number } {
  ...
  if (volumes.length > volLookback) {
    ...
    factors.volume = Math.max(-1, Math.min(1, (volumes[volumes.length - 1] / avgVol - 1) * 0.5));
  ...
  if (!isNaN(shortMA) && !isNaN(medMA)) {
    factors.breadth = Math.max(-1, Math.min(1, (current > shortMA ? 0.5 : -0.5) + (current > medMA ? 0.5 : -0.5)));
  ...
  const confidence = (Math.max(positiveFactors, negativeFactors) / factorValues.length) * Math.abs(score);
...
function runBacktest(genome: UltraGenome, barsMap: Map<string, AlpacaBar[]>, symbols: string[], startIdx: number, endIdx: number): BacktestResult {
      ...
      for (const c of candidates.slice(0, (genes.maxPositions || 20) - positions.size)) {
        ...
        if (shares > 0 && shares * c.price <= capital) {
          positions.set(c.symbol, {
            entry: c.price, shares, entryIdx: idx,
  ...
  for (const [symbol, pos] of positions) {
    ...
    if (symbolBars?.length) {
      ...
      trades.push({ pnl: (lastPrice - pos.entry) * pos.shares, holdingDays: endIdx - pos.entryIdx });
    }
  }
...
function calculateMetrics(equity: number[], dailyReturns: number[], trades: { pnl: number; holdingDays: number }[], regimeReturns: Record<string, number[]>, initialCapital: number): BacktestResult {
  ...
  const downStd = negReturns.length > 0 ? Math.sqrt(negReturns.reduce((sum, r) => sum + r * r, 0) / negReturns.length) : 1;
...
function calculateFitness(result: BacktestResult): number {
  ...
  return result.sharpe * 25 + result.sortino * 15 + result.calmar * 20 + result.winRate * 15 + result.totalReturn * 15 + (1 - result.maxDrawdown) * 10 + Math.min(result.profitFactor, 3) * 10 + Math.min(result.trades / 500, 1) * 5;
}
...
function generateUltraGenome(generation: number, island: number): UltraGenome {
  ...
  for (const [param, range] of Object.entries(PARAM_RANGES)) {
    ...
    let value = range.min + Math.floor(Math.random() * (steps + 1)) * range.step;
  ...
  return {
    ...
    genes, fitness: 0, sharpe: 0, sortino: 0, calmar: 0, winRate: 0, totalReturn: 0, maxDrawdown: 0, trades: 0,
    generation, island, parentIds: [], mutations: [], regime: 'unknown', evaluationTime: 0,
  };
}

function ultraCrossover(parent1: UltraGenome, parent2: UltraGenome, generation: number, island: number): UltraGenome {
...
function ultraMutate(genome: UltraGenome, mutationRate: number, learningEngine?: ContinuousLearningEngine): UltraGenome {
...
async function runUltraHyperoptimizer() {
  ...
  console.log(`  Configuration: ${CONFIG.POPULATION_SIZE} pop, ${CONFIG.NUM_ISLANDS} islands, ${ALL_SYMBOLS.length} symbols\n`);

  const prioritySymbols = ['SPY', 'QQQ', 'IWM', 'DIA', 'GLD', 'SLV', 'PPLT', 'NEM', 'GOLD', 'FCX', 'XME', 'XLF', 'XLK', 'XLE', 'XLV', 'XLI', 'XLP', 'XLY', 'XLB', 'XLU', 'XLRE', 'USO', 'XOM', 'CVX', 'DBA', 'DBB', 'CPER', 'AAPL', 'MSFT', 'GOOGL', 'AMZN', 'NVDA', 'META', 'TSLA', 'JPM', 'V', 'UNH', 'JNJ', 'RIO', 'BHP', 'VALE', 'AA', 'NUE', 'TLT', 'LQD', 'HYG', 'MTUM', 'QUAL', 'VLUE'];
  const symbolsToLoad = [...new Set(prioritySymbols)].slice(0, 60);

  const bars = await loadMarketData(symbolsToLoad, 5);
  if (bars.size < 10) { console.error('  Insufficient data. Aborting.\n'); return; }
  ...
  console.log(`  Trading: ${tradingSymbols.length} symbols, ${endIdx - startIdx} days\n`);
  ...
  const maxGenerations = Math.min(Math.ceil(CONFIG.TOTAL_ITERATIONS / CONFIG.POPULATION_SIZE), 50000);
  ...
  for (let generation = 0; generation < maxGenerations; generation++) {
    ...
    for (let islandIdx = 0; islandIdx < CONFIG.NUM_ISLANDS; islandIdx++) {
      ...
      while (newPopulation.length < island.length) {
        if (Math.random() < CONFIG.CROSSOVER_RATE) {
          const parent1 = tournamentSelect(island, CONFIG.TOURNAMENT_SIZE) as UltraGenome;
          const parent2 = tournamentSelect(island, CONFIG.TOURNAMENT_SIZE) as UltraGenome;
          let child = ultraCrossover(parent1, parent2, generation + 1, islandIdx);
          if (Math.random() < mutationRate) child = ultraMutate(child, mutationRate, learningEngine);
          newPopulation.push(child);
        } else {
          const parent = tournamentSelect(island, CONFIG.TOURNAMENT_SIZE) as UltraGenome;
          const mutant = ultraMutate(parent, mutationRate * 1.5, learningEngine);
    ...
    if (generation % CONFIG.PROGRESS_REPORT_INTERVAL === 0) {
      ...
      const diversity = new Set(allGenomes.map(g => JSON.stringify(g.genes))).size / allGenomes.length;
  ...
  if (globalBest && globalBestResult) {
    ...
    console.log(`  Trades: ${globalBestResult.trades}, WinRate: ${(globalBestResult.winRate * 100).toFixed(1)}%, PF: ${globalBestResult.profitFactor.toFixed(2)}`);
  }
