...
async function optimizeImage(inputPath: string, outputPath: string, resize?: { width: number; height: number }) {
  ...
  const reduction = ((originalSize - result.size) / originalSize * 100).toFixed(2);
