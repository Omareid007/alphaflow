...
function categorizeFailure(order, errorReason) {
  ...
  if (timeInForce === 'gtc' && (orderType === 'market' || lowerError.includes('gtc'))) {
    return {
  ...
  // Price issues
  if (lowerError.includes('limit_price') || lowerError.includes('price') ||
      lowerError.includes('stop_price') || lowerError.includes('invalid price')) {
    return {
  ...
  // Quantity issues
  if (lowerError.includes('qty') || lowerError.includes('quantity') ||
      lowerError.includes('notional') || lowerError.includes('fractional')) {
    return {
  ...
  if (lowerError.includes('symbol') || lowerError.includes('asset') ||
      lowerError.includes('not found') || lowerError.includes('not tradable')) {
    return {
  ...
  // Insufficient funds
  if (lowerError.includes('insufficient') || lowerError.includes('buying power') ||
  ...
  if (lowerError.includes('order_class') || lowerError.includes('bracket') ||
      lowerError.includes('oco') || lowerError.includes('oto')) {
    return {
...
async function analyzeOrderFailures() {
  ...
  try {
    ...
    if (orders.length === 0) {
      console.log('No canceled or rejected orders found in the specified time window.');
      return;
    }
    ...
    orders.forEach(order => {
      ...
      if (!orderTypeFailures.has(orderTypeKey)) {
        orderTypeFailures.set(orderTypeKey, { count: 0, categories: new Set() });
      }
    ...
    const sortedCategories = Array.from(categorizedFailures.values())
    ...
    sortedCategories.forEach((cat, idx) => {
      ...
      cat.examples.forEach((ex, i) => {
        console.log(`      ${i + 1}. ${ex.symbol} - ${ex.type}/${ex.tif} (Order: ${ex.orderId})`);
        console.log(`         Extended Hours: ${ex.extendedHours}, Order Class: ${ex.orderClass || 'simple'}`);
        console.log(`         Time: ${new Date(ex.submittedAt).toISOString()}`);
        console.log(`         Error: ${ex.error.substring(0, 120)}${ex.error.length > 120 ? '...' : ''}`);
      });
    });
    ...
    sortedSymbols.forEach(([symbol, data]) => {
      ...
      console.log(`${symbol.padEnd(10)} - ${data.count} failures (${categories})`);
    });
    ...
    const sortedTimes = Array.from(timeOfDayFailures.entries())
      .sort((a, b) => b[1].count - a[1].count);
    ...
    const sortedOrderTypes = Array.from(orderTypeFailures.entries())
      .sort((a, b) => b[1].count - a[1].count);

    sortedOrderTypes.forEach(([type, data]) => {
      const categories = Array.from(data.categories).join(', ');
      console.log(`${type.padEnd(20)} - ${data.count} failures (${categories})`);
    });
