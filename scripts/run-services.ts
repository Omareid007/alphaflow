...
const SERVICES: ServiceConfig[] = [
  { name: 'api-gateway', path: 'services/api-gateway/index.ts', port: 5000, color: '\x1b[36m' },
  { name: 'trading-engine', path: 'services/trading-engine/index.ts', port: 3001, color: '\x1b[32m' },
  { name: 'ai-decision', path: 'services/ai-decision/index.ts', port: 3002, color: '\x1b[33m' },
  { name: 'market-data', path: 'services/market-data/index.ts', port: 3003, color: '\x1b[34m' },
  { name: 'analytics', path: 'services/analytics/index.ts', port: 3004, color: '\x1b[35m' },
  { name: 'orchestrator', path: 'services/orchestrator/index.ts', port: 3005, color: '\x1b[31m' },
];
...
async function main(): Promise<void> {
  ...
  const selectedServices = args.length > 0
  ...
  console.log('\n' + '═'.repeat(60));
  console.log(`  ${processes.length}/${selectedServices.length} services started`);
