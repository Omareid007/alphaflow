...
async function testStock(ticker: string) {
  ...
  try {
    ...
    if (summary) {
      ...
      console.log(
        `  Buys: ${summary.totalInsiderBuys.toLocaleString()} shares`
      );
      console.log(
        `  Sells: ${summary.totalInsiderSells.toLocaleString()} shares`
      );
      console.log(`  Net Value: $${summary.netInsiderValue.toLocaleString()}`);
      console.log(`  Sentiment: ${summary.sentiment.toUpperCase()}`);
      console.log(
        `  Recent Transactions: ${summary.recentTransactions.length}`
      );
      ...
      if (summary.recentTransactions.length > 0) {
        ...
        console.log(
          `  ${latest.transactionType === "P" ? "BOUGHT" : latest.transactionType === "S" ? "SOLD" : latest.transactionType}`
        );
        console.log(
          `  ${latest.sharesTransacted.toLocaleString()} shares @ ${latest.pricePerShare ? "$" + latest.pricePerShare.toFixed(2) : "N/A"}`
        );
        console.log(
          `  Date: ${latest.transactionDate.toISOString().split("T")[0]}`
        );
      }
    } else {
