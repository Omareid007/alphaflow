...
function generateSignal(
  ...
  const regime = detectRegime(prices, index, indicators.sma20[index], indicators.sma50[index], indicators.adx[index]);

  const technical = calculateTechnicalScore(
    indicators.rsi[index], indicators.macd.histogram[index],
    indicators.stoch.k[index], indicators.stoch.d[index],
    price, indicators.bb.upper[index], indicators.bb.lower[index], indicators.bb.middle[index], config
  );

  const momentum = calculateMomentumScore(prices, index, indicators.emaFast[index], indicators.emaSlow[index], indicators.sma20[index]);
  const volatility = calculateVolatilityScore(indicators.adx[index], indicators.bb.upper[index], indicators.bb.lower[index], indicators.bb.middle[index]);
  ...
  return { technical, momentum, volatility, volume, sentiment, composite, confidence, regime };
}
...
function runBacktest(dataMap: Map<string, AlpacaBar[]>, config: BacktestConfig): BacktestResult {
  const trades: Trade[] = [];
  ...
  for (const date of sortedDates) {
    ...
    for (const [symbol, bars] of dataMap) {
      ...
      if (position) {
        ...
        else if (signals.composite < -config.sellThreshold && signals.confidence > config.confidenceMinimum) { exitReason = "signal_reversal"; }
        else if (bar.c > position.entryPrice * 1.02) {
          const atr = indicators.atr[dateIndex];
          if (atr !== null) position.stopLoss = Math.max(position.stopLoss, bar.c - atr * config.atrMultiplierStop);
        }
        ...
        if (exitReason) {
          ...
          trades.push({
            symbol, entryDate: position.entryDate, entryPrice: position.entryPrice,
            exitDate: date, exitPrice, shares: position.shares, side: position.side,
            pnl, pnlPct, exitReason, holdingDays, signals: position.signals,
          });
        ...
        if (canEnter) {
          ...
          if (shares > 0 && atr !== null) {
            positions.set(symbol, {
              symbol, shares, entryPrice: bar.c, entryDate: date,
    ...
    if (dailyPnl < -equity * config.maxDailyLoss) {
      for (const [symbol, position] of positions) {
        ...
        if (bar) {
          ...
          trades.push({
            symbol, entryDate: position.entryDate, entryPrice: position.entryPrice,
            exitDate: date, exitPrice: bar.c, shares: position.shares, side: position.side,
  ...
  for (const [symbol, position] of positions) {
    ...
    if (bar) {
      ...
      trades.push({
        symbol, entryDate: position.entryDate, entryPrice: position.entryPrice,
        exitDate: lastDate, exitPrice: bar.c, shares: position.shares, side: position.side,
        pnl, pnlPct: (bar.c - position.entryPrice) / position.entryPrice * 100,
...
function calculateSignalStats(signals: SignalComponents[]): SignalStats {
  if (signals.length === 0) {
    return { avgTechnical: 0, avgMomentum: 0, avgVolatility: 0, avgVolume: 0, avgSentiment: 0, avgComposite: 0, avgConfidence: 0, regimeCounts: {} };
  }
...
async function main() {
  ...
  const configs: BacktestConfig[] = [
    { symbols, startDate, endDate, initialCapital: 100000, maxPositionPct: 0.05, maxPortfolioExposure: 0.6, rsiPeriod: 14, rsiOversold: 30, rsiOverbought: 70, smaPeriod: 20, emaPeriodFast: 12, emaPeriodSlow: 26, atrPeriod: 14, adxPeriod: 14, stochPeriod: 14, macdFast: 12, macdSlow: 26, macdSignal: 9, bbPeriod: 20, bbStdDev: 2, atrMultiplierStop: 2, atrMultiplierTarget: 3, maxDailyLoss: 0.05, buyThreshold: 0.25, sellThreshold: 0.25, confidenceMinimum: 0.4, technicalWeight: 0.35, momentumWeight: 0.25, volatilityWeight: 0.15, volumeWeight: 0.1, sentimentWeight: 0.15 },
    { symbols, startDate, endDate, initialCapital: 100000, maxPositionPct: 0.06, maxPortfolioExposure: 0.7, rsiPeriod: 14, rsiOversold: 25, rsiOverbought: 75, smaPeriod: 20, emaPeriodFast: 9, emaPeriodSlow: 21, atrPeriod: 14, adxPeriod: 14, stochPeriod: 14, macdFast: 12, macdSlow: 26, macdSignal: 9, bbPeriod: 20, bbStdDev: 2, atrMultiplierStop: 1.5, atrMultiplierTarget: 2.5, maxDailyLoss: 0.04, buyThreshold: 0.3, sellThreshold: 0.3, confidenceMinimum: 0.45, technicalWeight: 0.50, momentumWeight: 0.20, volatilityWeight: 0.10, volumeWeight: 0.10, sentimentWeight: 0.10 },
    { symbols, startDate, endDate, initialCapital: 100000, maxPositionPct: 0.07, maxPortfolioExposure: 0.6, rsiPeriod: 10, rsiOversold: 35, rsiOverbought: 65, smaPeriod: 20, emaPeriodFast: 8, emaPeriodSlow: 21, atrPeriod: 10, adxPeriod: 14, stochPeriod: 10, macdFast: 8, macdSlow: 17, macdSignal: 9, bbPeriod: 20, bbStdDev: 2, atrMultiplierStop: 2, atrMultiplierTarget: 4, maxDailyLoss: 0.05, buyThreshold: 0.2, sellThreshold: 0.2, confidenceMinimum: 0.35, technicalWeight: 0.25, momentumWeight: 0.40, volatilityWeight: 0.10, volumeWeight: 0.15, sentimentWeight: 0.10 },
    { symbols, startDate, endDate, initialCapital: 100000, maxPositionPct: 0.05, maxPortfolioExposure: 0.5, rsiPeriod: 14, rsiOversold: 30, rsiOverbought: 70, smaPeriod: 20, emaPeriodFast: 12, emaPeriodSlow: 26, atrPeriod: 14, adxPeriod: 14, stochPeriod: 14, macdFast: 12, macdSlow: 26, macdSignal: 9, bbPeriod: 20, bbStdDev: 2, atrMultiplierStop: 2, atrMultiplierTarget: 3, maxDailyLoss: 0.04, buyThreshold: 0.3, sellThreshold: 0.3, confidenceMinimum: 0.5, technicalWeight: 0.25, momentumWeight: 0.20, volatilityWeight: 0.10, volumeWeight: 0.15, sentimentWeight: 0.30 },
    { symbols, startDate, endDate, initialCapital: 100000, maxPositionPct: 0.04, maxPortfolioExposure: 0.4, rsiPeriod: 14, rsiOversold: 25, rsiOverbought: 75, smaPeriod: 20, emaPeriodFast: 12, emaPeriodSlow: 26, atrPeriod: 14, adxPeriod: 14, stochPeriod: 14, macdFast: 12, macdSlow: 26, macdSignal: 9, bbPeriod: 20, bbStdDev: 2.5, atrMultiplierStop: 2.5, atrMultiplierTarget: 3, maxDailyLoss: 0.03, buyThreshold: 0.35, sellThreshold: 0.35, confidenceMinimum: 0.55, technicalWeight: 0.30, momentumWeight: 0.20, volatilityWeight: 0.20, volumeWeight: 0.15, sentimentWeight: 0.15 },
    { symbols, startDate, endDate, initialCapital: 100000, maxPositionPct: 0.08, maxPortfolioExposure: 0.8, rsiPeriod: 10, rsiOversold: 35, rsiOverbought: 65, smaPeriod: 15, emaPeriodFast: 8, emaPeriodSlow: 21, atrPeriod: 10, adxPeriod: 10, stochPeriod: 10, macdFast: 8, macdSlow: 17, macdSignal: 9, bbPeriod: 15, bbStdDev: 1.5, atrMultiplierStop: 1.5, atrMultiplierTarget: 4, maxDailyLoss: 0.06, buyThreshold: 0.15, sellThreshold: 0.15, confidenceMinimum: 0.3, technicalWeight: 0.35, momentumWeight: 0.30, volatilityWeight: 0.10, volumeWeight: 0.15, sentimentWeight: 0.10 },
  ];
  ...
  for (const r of results) {
    console.log(`Iter ${r.iteration}: Score=${r.score.toFixed(2)}, WinRate=${r.result.metrics.winRate.toFixed(1)}%, PF=${r.result.metrics.profitFactor.toFixed(2)}, Sharpe=${r.result.metrics.sharpeRatio.toFixed(2)}`);
  }
}
