...
async function testEndpoint(
  ...
  try {
    ...
    if (response.ok) {
      logSuccess(`${endpoint} - Status: ${response.status}, Duration: ${duration}ms`);
      return {
      ...
      logWarning(`${endpoint} - Status: ${response.status} ${response.statusText}`);
      return {
        name: `${method} ${endpoint}`,
        success: false,
        duration,
    ...
    const duration = Date.now() - startTime;
    logError(`${endpoint} - ${error instanceof Error ? error.message : String(error)}`);
...
async function main() {
  ...
  if (passRate === 100) {
    ...
    const directFailed = results
      .filter(r => r.name.includes('localhost:5000') && !r.success)
      .length;
    const nextFailed = results
      .filter(r => r.name.includes('localhost:3000') && !r.success)
      .length;
  ...
  console.log('4. Use the API Debug Panel in your browser for interactive testing');
