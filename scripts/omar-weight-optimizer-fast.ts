...
function simulateBacktest(weights: WeightConfig): OptimizationResult {
  ...
  const balance =
    1 -
    Math.sqrt(
      Math.pow(techScore - 0.2, 2) +
        Math.pow(momoScore - 0.2, 2) +
        Math.pow(volScore - 0.1, 2) +
        Math.pow(volumeScore - 0.15, 2) +
        Math.pow(sentimentScore - 0.15, 2) +
        Math.pow(patternScore - 0.1, 2) +
...
function generateWeightVariations(): WeightConfig[] {
  ...
  for (let tech = 0.15; tech <= 0.35; tech += 0.05) {
    for (let momo = 0.15; momo <= 0.35; momo += 0.1) {
      variations.push(
        normalizeWeights({
...
async function main() {
  ...
  for (let i = 0; i < Math.min(20, results.length); i++) {
    ...
    console.log(
      `Sharpe: ${r.sharpe.toFixed(3)} | Sortino: ${r.sortino.toFixed(3)} | Calmar: ${r.calmar.toFixed(3)}`
    );
    console.log(
      `Win Rate: ${r.winRate.toFixed(1)}% | Return: ${r.totalReturn.toFixed(1)}% | Max DD: ${r.maxDrawdown.toFixed(1)}%`
    );
    console.log(
      `CAGR: ${r.cagr.toFixed(1)}% | Profit Factor: ${r.profitFactor.toFixed(2)} | Trades: ${r.totalTrades}`
    );
    console.log(`Weights:`);
    console.log(
      `  Technical:    ${(r.weights.technicalWeight * 100).toFixed(1)}%`
    );
    console.log(
      `  Momentum:     ${(r.weights.momentumWeight * 100).toFixed(1)}%`
    );
    console.log(
      `  Volatility:   ${(r.weights.volatilityWeight * 100).toFixed(1)}%`
    );
    console.log(
      `  Volume:       ${(r.weights.volumeWeight * 100).toFixed(1)}%`
    );
    console.log(
      `  Sentiment:    ${(r.weights.sentimentWeight * 100).toFixed(1)}%`
    );
    console.log(
      `  Pattern:      ${(r.weights.patternWeight * 100).toFixed(1)}%`
    );
    console.log(
      `  Breadth:      ${(r.weights.breadthWeight * 100).toFixed(1)}%`
    );
    console.log(
      `  Correlation:  ${(r.weights.correlationWeight * 100).toFixed(1)}%`
    );
  }
  ...
  console.log(
    `  technicalWeight:    ${best.weights.technicalWeight.toFixed(4)}`
  );
  console.log(
    `  momentumWeight:     ${best.weights.momentumWeight.toFixed(4)}`
  );
  console.log(
    `  volatilityWeight:   ${best.weights.volatilityWeight.toFixed(4)}`
  );
  console.log(`  volumeWeight:       ${best.weights.volumeWeight.toFixed(4)}`);
  console.log(
    `  sentimentWeight:    ${best.weights.sentimentWeight.toFixed(4)}`
  );
  ...
  console.log(
    `  correlationWeight:  ${best.weights.correlationWeight.toFixed(4)}`
  );
  ...
  if (avgWeights.technical + avgWeights.momentum > 0.45) {
    console.log(
      `  - Technical + Momentum dominate top performers (${((avgWeights.technical + avgWeights.momentum) * 100).toFixed(1)}%)`
    );
  }
  if (avgWeights.volume > 0.12) {
    console.log(
      `  - Volume confirmation is important (${(avgWeights.volume * 100).toFixed(1)}%)`
    );
  }
  if (avgWeights.pattern > 0.08) {
    console.log(
      `  - Pattern recognition adds alpha (${(avgWeights.pattern * 100).toFixed(1)}%)`
    );
  }
