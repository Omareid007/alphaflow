...
const SECTOR_STOCKS = {
  ...
  'XLK': ['AAPL', 'MSFT', 'NVDA', 'GOOGL', 'META', 'AVGO', 'ORCL', 'AMD', 'CRM', 'INTC'],
...
const OPTIMIZATION_CONFIGS = [
  ...
  { name: 'Pure_Sector_Top3_Weekly', type: 'pure_sector', topN: 3, rotationDays: 5 },
  { name: 'Pure_Sector_Top3_BiWeekly', type: 'pure_sector', topN: 3, rotationDays: 10 },
  { name: 'Pure_Sector_Top3_Monthly', type: 'pure_sector', topN: 3, rotationDays: 21 },
  { name: 'Pure_Sector_Top5_Weekly', type: 'pure_sector', topN: 5, rotationDays: 5 },
  { name: 'Pure_Sector_Top5_BiWeekly', type: 'pure_sector', topN: 5, rotationDays: 10 },
  { name: 'Pure_Sector_Top5_Monthly', type: 'pure_sector', topN: 5, rotationDays: 21 },

  // Mixed strategies (sector ETFs + stocks)
  { name: 'Mixed_Top2Sectors_Weekly', type: 'mixed', topN: 2, rotationDays: 5, stocksPerSector: 3 },
  { name: 'Mixed_Top2Sectors_BiWeekly', type: 'mixed', topN: 2, rotationDays: 10, stocksPerSector: 3 },
  { name: 'Mixed_Top3Sectors_Weekly', type: 'mixed', topN: 3, rotationDays: 5, stocksPerSector: 2 },
  { name: 'Mixed_Top3Sectors_BiWeekly', type: 'mixed', topN: 3, rotationDays: 10, stocksPerSector: 2 },

  // Momentum-based strategies
  { name: 'Momentum_Top3_5Day', type: 'momentum', topN: 3, rotationDays: 5, lookback: 20 },
  { name: 'Momentum_Top3_10Day', type: 'momentum', topN: 3, rotationDays: 10, lookback: 20 },
  { name: 'Momentum_Top5_5Day', type: 'momentum', topN: 5, rotationDays: 5, lookback: 20 },
  { name: 'Momentum_Top3_LongLookback', type: 'momentum', topN: 3, rotationDays: 10, lookback: 60 },

  // Relative strength strategies
  { name: 'RelativeStrength_Top3_Weekly', type: 'relative_strength', topN: 3, rotationDays: 5, benchmarkIndex: 'SPY' },
  { name: 'RelativeStrength_Top3_BiWeekly', type: 'relative_strength', topN: 3, rotationDays: 10, benchmarkIndex: 'SPY' },
  { name: 'RelativeStrength_Top5_Weekly', type: 'relative_strength', topN: 5, rotationDays: 5, benchmarkIndex: 'SPY' },

  // Regime-based strategies
  { name: 'Regime_Defensive_Monthly', type: 'regime', rotationDays: 21, regimeType: 'defensive' },
  { name: 'Regime_Cyclical_Monthly', type: 'regime', rotationDays: 21, regimeType: 'cyclical' },
  { name: 'Regime_Adaptive_BiWeekly', type: 'regime', rotationDays: 10, regimeType: 'adaptive' },
];
...
function scoreSectors(
  ...
  for (const { symbol, name } of SECTOR_ETFS) {
    ...
    const volatility = calculateVolatility(closes, Math.min(30, closes.length - 1));

    // Trend strength
    const trend = calculateTrend(closes);

    // Composite score
    const score = momentum * 0.4 + relativeStrength * 0.3 + (1 - volatility) * 0.2 + trend * 0.1;

    scores.push({ symbol, name, score, momentum, relativeStrength, volatility, trend });
  }
...
async function runSectorRotationBacktest(
  ...
  const trades: { entry: number; exit: number; symbol: string; days: number }[] = [];
  ...
  let positions: Map<string, { entry: number; shares: number; entryDate: string }> = new Map();
    ...
    if (daysSinceRotation >= config.rotationDays || positions.size === 0) {
      ...
      if (config.type === 'pure_sector' || config.type === 'momentum') {
        ...
        for (const { symbol } of SECTOR_ETFS) {
          const symbolBars = bars.get(symbol);
          if (symbolBars) {
            barsToDate.set(symbol, symbolBars.filter(b => b.t <= currentDate));
          }
        }
        const benchmarkBars = spyBars.filter(b => b.t <= currentDate);
        const scores = scoreSectors(barsToDate, benchmarkBars, config.lookback || 20);
        ...
        for (const sector of topSectors) {
          const stocks = SECTOR_STOCKS[sector.symbol as keyof typeof SECTOR_STOCKS] || [];
          ...
          if (symbolBars) {
            ...
            if (currentBar && currentBar.c > 0) {
              ...
              if (shares > 0) {
                ...
                const sectorSymbol = config.type === 'mixed'
                  ? Object.keys(SECTOR_STOCKS).find(k => (SECTOR_STOCKS[k as keyof typeof SECTOR_STOCKS] || []).includes(symbol)) || symbol
                  : symbol;
                sectorExposureCount[sectorSymbol] = (sectorExposureCount[sectorSymbol] || 0) + 1;
              }
            }
          }
        }
  ...
  const sortedSectors = Object.entries(sectorExposureCount).sort((a, b) => b[1] - a[1]);
...
async function main() {
  ...
  for (let i = 0; i < OPTIMIZATION_CONFIGS.length; i++) {
    ...
    process.stdout.write(`  [${i + 1}/${OPTIMIZATION_CONFIGS.length}] ${config.name}...`);

    try {
      const result = await runSectorRotationBacktest(config, bars, startDate, endDate);
      results.push(result);
      console.log(` ✓ Sharpe: ${result.sharpe.toFixed(2)}, Return: ${(result.totalReturn * 100).toFixed(1)}%`);
    } catch (err) {
  ...
  console.log('  Rank | Configuration                    | Sharpe | Sortino | Calmar | Win% | Return  | MaxDD   | Trades');
  ...
  if (results.length > 0) {
    ...
    console.log(`  Annualized Return:   ${(best.annualizedReturn * 100).toFixed(2)}%`);
    ...
    console.log(`\n  Sector Exposure:`);
    const sortedExposure = Object.entries(best.details.sectorExposure)
      .sort((a, b) => b[1] - a[1]);
    for (const [sector, count] of sortedExposure.slice(0, 10)) {
      const sectorName = SECTOR_ETFS.find(s => s.symbol === sector)?.name || sector;
      console.log(`    ${sector.padEnd(6)} (${sectorName.padEnd(20)}): ${count} rotations`);
    }
  ...
  for (const result of results) {
    const config = OPTIMIZATION_CONFIGS.find(c => c.name === result.configName);
    if (config) {
  ...
  for (const [type, typeResults] of Object.entries(byType)) {
    const avgSharpe = typeResults.reduce((sum, r) => sum + r.sharpe, 0) / typeResults.length;
    const avgReturn = typeResults.reduce((sum, r) => sum + r.totalReturn, 0) / typeResults.length;
    const avgMaxDD = typeResults.reduce((sum, r) => sum + r.maxDrawdown, 0) / typeResults.length;
