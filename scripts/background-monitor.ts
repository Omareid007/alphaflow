  ...
  try {
    ...
    for (const pos of positions) {
      ...
      const warningThreshold =
        config.MAX_POSITION_PCT * (config.WARNING_THRESHOLD / 100);
      ...
      if (concentration >= config.MAX_POSITION_PCT) {
        ...
        await sendAlert({
          ...
          threshold: config.MAX_POSITION_PCT,
          recommendedAction:
            "Monitor position - consider trimming on next rally",
        });
      }
    }
  } catch (e) {
...
async function checkSectorExposure(): Promise<void> {
  try {
    ...
    for (const [sector, value] of Object.entries(sectorExposure)) {
      ...
      const warningThreshold =
        config.MAX_SECTOR_PCT * (config.WARNING_THRESHOLD / 100);
...
async function checkDrawdown(): Promise<void> {
  try {
    ...
    const warningThreshold =
      config.MAX_DAILY_DRAWDOWN * (config.WARNING_THRESHOLD / 100);
    ...
    if (dailyDrawdown >= config.MAX_DAILY_DRAWDOWN) {
      await sendAlert({
        ...
        recommendedAction:
          "HALT TRADING - Review positions and consider closing losers",
      });
...
async function checkCircuitBreaker(): Promise<void> {
  try {
    const response = await fetch(
      `${config.SERVER_URL}/api/alpaca/circuit-breaker/status`
    );
    ...
    if (data.isOpen) {
      await sendAlert({
        ...
        threshold: 5,
        recommendedAction:
          "Wait for auto-reset (60s) or manually reset via API",
      });
    }
  } catch {
...
async function startMonitoring(): Promise<void> {
  ...
  setInterval(() => {
    console.log(
      `[${new Date().toISOString()}] Monitor heartbeat - all checks running`
    );
  }, 300000);
}
