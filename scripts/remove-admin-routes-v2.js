...
for (let i = 0; i < lines.length; i++) {
    ...
    const routeMatch = line.match(/^\s*app\.(get|post|put|patch|delete)\s*\(\s*["']\/api\/admin/);
    ...
    if (routeMatch) {
        ...
        for (let j = i; j < lines.length; j++) {
            ...
            if (j - i > 200) {
                console.log('Warning: Could not find end of route at line', startLine + 1, 'after 200 lines');
