...
function adminTokenMiddleware(req: Request, res: Response, next: NextFunction) {
  ...
  if (!isProduction) {
    console.log("[Auth] Development mode - bypassing admin token check for:", req.method, req.path);
    req.userId = "dev-user";
    return next();
  }
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/strategies/:id/deploy", async (req, res) => {
    try {
      const { mode } = req.body;
    ...
    try {
      ...
      if (strategy.isActive) {
        ...
        if (!stopResult.success) {
          console.warn(`Warning: Could not stop strategy before deletion: ${stopResult.error}`);
        }
      }
  ...
  // Portfolio snapshot endpoint - combines account data and positions for portfolio overview
  app.get("/api/positions/snapshot", async (req, res) => {
    ...
    try {
      ...
      const transformedPositions = filteredPositions.map(p => ({
        id: p.asset_id || `pos-${p.symbol}`,
        symbol: p.symbol || '',
        ...
        assetClass: p.asset_class === 'crypto' ? 'crypto' : 'us_equity'
      }));
      ...
      const snapshot = {
        ...
        dailyPl: parseFloat(account.equity || '0') - parseFloat(account.last_equity || account.equity || '0'),
        dailyPlPct: ((parseFloat(account.equity || '0') - parseFloat(account.last_equity || account.equity || '0')) / parseFloat(account.last_equity || account.equity || '1')) * 100,
        totalPl: parseFloat(account.equity || '0') - parseFloat(account.initial_margin || account.equity || '0'),
        totalPlPct: ((parseFloat(account.equity || '0') - parseFloat(account.initial_margin || account.equity || '0')) / parseFloat(account.initial_margin || account.equity || '1')) * 100,
      ...
      res.status(503).json({
        ...
        message: "Could not fetch portfolio data from Alpaca. Please try again shortly."
      });
    }
  });
  ...
  // Alias for frontend compatibility - /api/decisions points to /api/ai-decisions
  ...
  app.get("/api/ai/events", async (req, res) => {
    try {
      ...
      // Get recent AI decisions as events
      const decisions = await storage.getAiDecisions(limit + offset);
      const events = decisions.slice(offset, offset + limit).map((decision) => ({
        id: decision.id,
        type: "ai_decision",
        timestamp: decision.createdAt,
        symbol: decision.symbol,
        ...
        metadata: {
          ...
          executedTradeId: decision.executedTradeId,
        },
      }));

      // Get orchestrator events if available
      const orchestratorState = orchestrator.getState();
      const orchestratorEvents = orchestratorState.executionHistory
        ?.slice(0, 20)
        .map((item: any) => ({
          id: `orch-${item.timestamp || Date.now()}`,
      ...
      const allEvents = [...events, ...orchestratorEvents]
        .sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime())
        .slice(0, limit);

      res.json({
        events: allEvents,
        total: events.length + orchestratorEvents.length,
        hasMore: offset + limit < decisions.length,
      });
    } catch (error) {
      console.error("Failed to get AI events:", error);
      res.status(500).json({ error: "Failed to get AI events" });
    }
  });
